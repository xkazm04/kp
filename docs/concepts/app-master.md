...
| **P2** | `repo_scan` task + `claude_cli` cwd/allowed-tools support + keyless walker + `repo_scans` store | `app/_lib/repo-scan*.ts`, `pipeline/jobfit/repo_scan*.py`, `tasks.ts`, `claude_cli.py`, tenancy + contract tests | dossier on kp itself matches `context-map.json` (143 contexts) |
