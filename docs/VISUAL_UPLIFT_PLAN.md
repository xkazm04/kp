...
- [ ] Declare the spacing rhythm in DESIGN.md (section `space-y-6`, intra-panel
      `space-y-3`, eyebrow→title `mt-1`, title→intro `mt-2`) — convention only.
- [ ] `TAB_HEADER` wrapper + `TABLE` (th/tr/td) recipes (added when Phase 2 needs
      them).
...
## Phase 2 — Roll composition across tabs  ⏳ next
- [ ] **Headline:** unify tab structure — header above, content in panels on
      cream. Fix the "whole tab in one white card" pattern (`JobsTab.tsx:78`);
      adopt `PAGE_HEADER` + `SECTION` everywhere.
...
- [ ] `TABLE` recipe across AnalyticsTab tables (`:363`, `:602`).
...
## Phase 4 — Selective delight (low-risk zones only)  ⏳
...
- Hand font app-wide (payload); warm cream paper override (token fragmentation).
