...
| 3a | The GitHub deep-dive stage (`app/_lib/analyze-github-stage.ts`), only when the form carried a handle and the run is **not blind** | The handle is on the task params, but only for a caller with `pipeline:write` and within the `github-analysis:<ip>` budget. A blind run never forwards it, and the stage refuses blind on its own. A file-only JD is extracted from its persisted path (`pipeline.jobfit.extract_cli`). The result is attached to the saved row (`analyses.github_json`) by the task, not by the browser. | **Yes**: the handle goes to `api.github.com` and public repo signals go to the Gemini deep review. The CV itself never does. |
| 4 | `spawnPython(["-m", "pipeline.jobfit.cli", cvPath, …])` (`app/_lib/analyze-run.ts`) | A **child process** with the CV's *path* in `argv`, the parent's env, and `KP_LLM_CONFIG` (`env: buildLlmConfigEnv()`) — the routing table + keys the child's `resolve_provider("cv_analysis")` reads. Until 2026-09-05 this spawn passed **no** env fragment, so a `cv_analysis` row chosen in Settings → Models and any BYOM key were inert on this path and the built-in default always served it; the destination in hop 5 is now the one the operator picked. Pinned by `app/_lib/llm-spawn-contract.test.ts`. Pasted JD/company text over 8 KB is spilled to a file rather than passed inline, so it never lands in a command line. | No — but it is where hop 5 becomes possible |
...
| Sentry, if `SENTRY_DSN` is set | Per your Sentry retention | Off by default. **Candidate capability tokens are redacted before egress** by a `beforeSend`/`beforeBreadcrumb` pair in `instrumentation.ts` / `instrumentation-client.ts` — without it a single error on `/data/<erasureToken>` or `/stop/<token>` shipped a working link to a third party. Keep the token-prefix lists in both files in sync. |
