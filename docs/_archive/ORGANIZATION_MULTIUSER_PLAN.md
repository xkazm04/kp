> **Archived 2026-07-30.** This was a 2026-07-05 impact-analysis/plan; the E0
> tenancy + identity work it scoped has since **shipped on `main`** (verified:
> `app/_lib/db/{organizations,users,memberships,invites}.ts`,
> `app/_lib/auth/roles.ts`, `app/_lib/tenancy.ts` shows `tenancyGaps()` at
> zero). Superseded by `docs/features/organization/README.md`, which documents
> the shipped reality and the remaining last-mile gaps. Kept here as the
> original impact-analysis record.
