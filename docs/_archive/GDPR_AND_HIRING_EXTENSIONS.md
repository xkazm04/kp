> **Archived 2026-07-30.** This was the design/build plan for 7 features, all
> now shipped (see its own Status section). The compliance-relevant parts
> (#1 consent, #2 erasure, #3 consent-audit) are folded into
> `docs/features/compliance/README.md`. The other 4 (#4 analytics-stage-dwell,
> #5 pool-fit, #6 onboarding hand-off, #7 NL command bar) belong to other
> feature areas (analytics, talent pipeline, onboarding, pipeline commands)
> outside this group's ownership and were not re-homed by this pass — flagged
> as a follow-up so another feature-doc owner picks them up. Kept for
> historical/design-rationale value (the DPO note on score retention is still
> load-bearing).
