> **Archived 2026-07-30.** Competitive gap analysis dated 2026-06-11; every
> P1/P2/P3 item except E6 (candidate-language expansion, deferred pending a
> product decision) shipped the same day and remains verified in code:
> campaign packs (`pipeline/jobfit/campaign.py`, `docs/features/jobs/README.md`),
> quick-apply (`app/apply/[id]/quick`), inbound webhooks (`app/api/channels/**`),
> source attribution/economics (`app/_lib/source-analytics.ts`), the JD lint
> (`app/_lib/jd-lint.ts`), and the `kp.comm.v1` export schema
> (`docs/features/comms/outbound-export.md`). Kept for the competitive
> research; E6 carried forward as an open backlog item.
