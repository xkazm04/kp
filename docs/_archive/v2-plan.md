...
> **ARCHIVED 2026-07-30.** Written 2026-05-29 as the design doc for turning the v1
> single-analysis app into a real matching platform with student/career-switcher support.
> **Verdict: shipped.** Verified against code at archive time — the 3-layer matching
> engine (`pipeline/jobfit/matching.py`: `ko_filter` → `score_skills`/`score_career`/
> `score_personal` → confidence), the archetype router (`pipeline/jobfit/archetype.py`:
> `BAU`/`STUDENT`/`CAREER_SWITCHER`), job ingestion with a precomputed entry lens
> (`pipeline/jobfit/jobs.py`: `ingest_raw_ad`, `JobEntryProfile.is_entry_eligible`/
> `graduate_friendliness`), the evidence/potential transformation (`transform.py`), the
> transferable-skill mapper for career-switchers (`transferable.py`), and the semantic
> bridge (`embedding_bridge.py`) are all real, tested, and wired into the apply flow
> (`app/apply/[id]/ConversationalApply.tsx` carries "per-archetype lanes (student /
> switcher / experienced)") and the recruiter UI (`app/features/tools/profile/*`,
> `ArchetypeManager.tsx`, `archetype-registry.ts`). This document is now a historical
> design record, not a live plan — do not treat any "not built yet" language below as
> current; check the files above instead.
>
> **What did NOT ship**, and remains open: this plan's §12 framing of the *candidate*
> as a self-service platform user (a true two-sided product where students/switchers log
> in and manage their own profile) never materialized — the archetype/student machinery
> is used by recruiters and the `apply` intake flow, not by a candidate-facing app. That
> gap is tracked as **W7 — Two-sided / candidate mode** in
> `docs/product/coverage-plan.md` (deferred, not scheduled). No feature doc exists yet for
> the matching engine itself — see the coverage-plan report's backlog note.
>
> **Purpose** (original, 2026-05-29). Two things at once:
