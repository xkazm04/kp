...
| **R-52** | **DONE.** A candidate-reachable opt-out, end to end: a separately scoped token (so neither key opens the other lock), a one-click  page that states it stops outreach and erases nothing, and a halt recorded distinctly from the operator halt. It resolves at the **durable candidate identity**, so a fresh rediscovery entry inherits it rather than re-arming contact, and it is fail-closed where its workflow-state siblings are fail-open. Enforced at the shared send precondition rather than per call site, and rediscovery filters at rank time so an opted-out person is never ranked or offered a reach-out button. The mail header is carried pre-formatted in the relay envelope rather than set on the relay request, where no mail agent would have read it. | , , ,  |
