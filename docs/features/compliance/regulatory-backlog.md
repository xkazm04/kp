...
| **R-06** | **DONE.** The candidate disclosure's regime and retention window are resolved **server-side from the token's own workspace** and passed as props; all seven public surfaces are threaded, each from the workspace behind its own token or job. The route stays gated, which is the substantive call: allow-listing it would fix nothing, because an anonymous request carries no workspace, and the only way to make it tenant-aware for a public caller is a caller-supplied workspace id that would let anyone enumerate any team's legal posture. One session-bearing render site keeps the fetch with its reason recorded. Pinned structurally, so an eighth candidate surface cannot quietly revert to asserting EU law. | `compliance-disclosure.ts`, `AiDisclosure.tsx`, 7 surfaces, `ai-disclosure-props.test.ts` |
| **R-19** | **DONE**, and it grew. Stating the prompt-free telemetry posture publicly turned up the one field that could break it: the emitter forwarded up to 500 bytes of provider-authored error text, where a provider message can echo the prompt and here a prompt carries a CV. The Python half already collapsed prose to a code before it reached a durable column; the TypeScript half did not, so the discipline held on one runtime out of two. It now applies the same rule. | `llm-lighttrack.ts` |
| **R-41** | **DONE.** Ten provider-binding articles added to the conformity map — Art. 4, 5, 17, 18, 19, 20, 21, 25, 43, 47/48 — with Art. 50 split into its two limbs, and seven new gaps (G17–G23). Art. 17 turned out to be **partial rather than absent**: most of a quality management system is already running and had simply never been called one. Its one genuinely empty head is the accountability framework, because Art. 17(1)(m) wants named people and kp's accountability is machine-shaped. Art. 20 has a structural blocker rather than a procedural one — under AGPL distribution there is no register of deployers to inform. | `ai-act-conformity.md` |
| **R-46** | **DONE.** The no-demographic-data posture is written up as a deliberate choice: what it buys (no exceptional-circumstances burden under the tightened Art. 10(5), no special-category data to defend), how fairness is tested instead (perturbation, not collection), and what it costs — kp **cannot** measure its own disparate impact, and perturbation testing does not prove otherwise. Ends with what would reopen it. | `ai-act-conformity.md`, `compliance/README.md` |
| **R-47** | **DONE (claim half).** Art. 86 added to the public table. The claim worth making is not that the explanation exists but that it is **derived from the sealed record rather than regenerated**, which is what stops the candidate's account and the auditor's account drifting apart; a test pins that property. Posture is partial, and the gap is the narrow real one — an unresolved actor renders no attribution line — rather than the operator-gating, which is a design choice. | `trust-posture.ts` |
| **R-63** | **NEW, and DONE.** Found while stating the telemetry posture: the no-egress seal's two halves disagreed and the TypeScript half was the permissive one. Python has always refused a configured endpoint that resolves to a public host, in as many words; the TypeScript allowlist admitted every configured endpoint unconditionally while its own comment called them private. So `KP_OFFLINE` had a route out through the variable an operator sets to describe where their private service lives. The private-host predicate is now ported and applied to the four outbound endpoints, with the app's own origin exempt because reaching yourself is not egress. One behaviour change, documented: a hosted Azure endpoint is public and is now sealed. `self-hosting.md` also loses a sentence claiming Azure keeps working under offline unconditionally, which was never what the adapter did. | `offline.ts`, `self-hosting.md` |
| **R-52** | **DONE.** A candidate-reachable opt-out, end to end: a separately scoped token (so neither key opens the other lock), a one-click `/stop/[token]` page that states it stops outreach and erases nothing, and a halt recorded distinctly from the operator halt. It resolves at the **durable candidate identity**, so a fresh rediscovery entry inherits it rather than re-arming contact, and it is fail-closed where its workflow-state siblings are fail-open. Enforced at the shared send precondition rather than per call site, and rediscovery filters at rank time so an opted-out person is never ranked or offered a reach-out button. The mail header rides pre-formatted in the relay envelope rather than being set on the relay request, where no mail agent would have read it. | `app/stop/`, `comms.ts`, `rediscover.ts`, `outreach-state-store.ts` |
...
| **R-30** | **PARTIAL.** `expiring_notified` now has a writer: `notifyExpiringConsents` (instrumentation heartbeat, sibling to the anonymize sweep) claims the event once and enqueues a `consent_expiry` letter with the `/data` and `/stop` footers. `erasure_requested` still has **no writer**. Implementing the reminder is also the natural place to ask for talent-pool consent (R-21), which this pass does not. | Both. | Both. | S |
