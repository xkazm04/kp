...
policy versions. The same operator-session re-verify is pinned for every
...
`requireOperator` on either is a red test rather than a public PII leak. A **separate, redacted candidate-facing view** now exists:
