...
| **Art. 86** Explanation of individual decisions | 🟡 (improved) | The sealed per-candidate dossier exists (`GET /api/decisions/records?candidate=…`) but stays operator-gated by design. **New:** `app/_lib/status-decisions.ts` derives a redacted `CandidateDecisionView` (kind, attribution, reasonCode, and the decisive `facts`) from the same sealed rows and renders it on `/status/[token]`; rejection copy is sourced from this record rather than generated. `facts` is a closed discriminated union with one extractor per sealed kind, and its coverage is a **counted ratio**: `factsCoverage()` reads 2 of 14 visible kinds and **2 of the 5 AI-verdict kinds** (`auto_rejected` → the decisive score-vs-cutoff, `ai_scorecard` → the assessed competencies and their ratings), asserted in `app/api/status/status-decisions.test.ts`. Still bare: `auto_advanced`, `group_eval_lead`, `group_eval_advisory`. | G9 now **partially** closed — candidates get a structured explanation of their own decision, with decisive elements on 2 of the 5 kinds where a machine judged them; they still cannot browse the full sealed chain (by design, not a gap) |
...
| G9 | Candidate-facing explanation of an individual decision. | 86, 13 | Both | M | **Partially closed** — `app/_lib/status-decisions.ts` + `/status/[token]` now render a redacted per-decision explanation (kind, attribution, reason, and decisive `facts` on `auto_rejected` + `ai_scorecard`). Coverage is counted, not asserted: `factsCoverage()` = 2 of 5 AI-verdict kinds. Full sealed dossier remains operator-only by design, not by gap. |
...
- **G9 was fully open, now partially closed**: `app/_lib/status-decisions.ts`
  ...
  is squarely an Art. 86 candidate-explanation mechanism). **How much of G9 is
  closed is now a number rather than a word**: `factsCoverage()` counts the
  sealed kinds that can show a decisive element (2 of 14 visible, 2 of 5
  AI-verdict), and `npm run kpi:reasons` counts the other half of the same
  question — whether the verdicts this product *produces* carry a reasons block
  at all. Both are described in
  [`docs/features/compliance/README.md`](./README.md).
