...
| proof.cards.sealed / trust.audit | tamper-evident chain naming the human | FRESH mechanism; attribution **CLOSED for the bulk wave** — a screening-wave commit whose approver cannot be named is refused rather than sealed (`isNamedApprover()` / `NAMED_APPROVER_REQUIRED` in `app/_lib/auth/operator-approver.ts`, gated at `app/_lib/screen-wave.ts`), and the audit table badges the historical role-only records instead of rewriting them (`DecisionRecordsTable.tsx`). Residual: single-candidate seals stay role-attributed by design. The `key_id=''` half is a different fact and is already surfaced — the panel's key census names it (08-17 UAT, 66 records, `08-17/SUMMARY.md:73`) |
...
| trust.human.body | "No candidate is advanced, offered… by the machine alone… not a setting" | **FIXED** — the landing now claims the rejection absolute plus "human-approved by default", pinned to `INTERVIEW_PLAN_DEFAULT` and the two real `getPlanGateForRole` branches (`MarketingClaims.test.ts`). `trust-posture.ts`'s Art. 14 row carried the same retired absolute for one commit longer and now states the same shape, pinned off the same modules (`trust-posture.test.ts`); `HumanLoopArt`'s toggles argue screening/offer configurable, rejection locked |
