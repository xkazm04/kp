...
export async function saveRoutingPin(
  ...
  try {
    ...
    if (!r.ok || !p.rows) throw new Error(p.error || fallbackMessage);
...
export async function resetRoutingPin(useCase: string, fallbackMessage: string): Promise<RoutingActionResult> {
  try {
    const r = await fetch("/api/llm/config", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
