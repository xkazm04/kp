...
export function ReasoningPanel({ state }: { state: ReasoningState }) {
  if (state.loading) return <p className="mt-3 text-sm text-steel">Generating reasoning…</p>;
  if (state.error) return <p className="mt-3 rounded-md bg-red-50 p-2 text-sm text-red-700">{state.error}</p>;
