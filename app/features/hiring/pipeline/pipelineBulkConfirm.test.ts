...
const SCOPE = "scope-a";
// ...and the signature of the cohort the confirm names (cohort-drift-forces-a-fresh-review).
const COHORT = "reject|a:Screened:screening_review";

test("a confirm is armed only for the cohort it SIGNED - a drift in the named people re-arms", () => {
  for (const armed of ["reject", "outreach"] as const) {
    const state = bulkConfirmReducer(null, { type: "arm", which: armed, scope: SCOPE, cohort: COHORT });
    assert.equal(armedConfirm(state, SCOPE, COHORT), armed);
    assert.equal(armedConfirm(state, SCOPE, "reject|a:Interview:scorecard_review"), null, "same scope, different people");
  }
});
...
test("selectionChanged disarms EITHER armed confirm — the defect", () => {
  ...
  for (const armed of ["reject", "outreach"] as const) {
    const state: BulkConfirm = bulkConfirmReducer(null, { type: "arm", which: armed, scope: SCOPE, cohort: COHORT });
    assert.equal(armedConfirm(state, SCOPE, COHORT), armed, "arming sets the confirm");
    assert.equal(
...
test("arming one confirm disarms the other (single-slot state)", () => {
  const reject = bulkConfirmReducer(null, { type: "arm", which: "reject", scope: SCOPE, cohort: COHORT });
  assert.equal(armedConfirm(bulkConfirmReducer(reject, { type: "arm", which: "outreach", scope: SCOPE, cohort: COHORT }), SCOPE, COHORT), "outreach");
  const outreach = bulkConfirmReducer(null, { type: "arm", which: "outreach", scope: SCOPE, cohort: COHORT });
  assert.equal(armedConfirm(bulkConfirmReducer(outreach, { type: "arm", which: "reject", scope: SCOPE, cohort: COHORT }), SCOPE, COHORT), "reject");
});
