 ...
 * that has any (only the picked role's lanes when a role is picked), and lists them with the stage
 ...
 */
export function useRejectedShelf(active: boolean, rejectedByLane: Record<string, number>, entries: readonly Entry[], role: string | null): Shelf & { retry: () => void } {
  const [shelf, setShelf] = useState<Loaded>(IDLE);
  const [tick, setTick] = useState(0);
  const lanes = Object.keys(rejectedByLane)
    .filter((lane) => rejectedByLane[lane] > 0)
    .filter((lane) => !role || entries.some((e) => (e.jobTitle ?? "") === role && entryLaneKey(e) === lane) || lane === role)
    .sort();
