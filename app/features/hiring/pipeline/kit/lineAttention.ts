...
// entry's own approval flag. The roles board (PipelineKitRoles) counts the AI's share per
// role on the row's second line, so a glance down the board answers "where is a human
// needed?" and "where is the machine still working?". Restored from the retired Subway
// board (b7fde0c32) with its tests: the rule is unchanged.
//
