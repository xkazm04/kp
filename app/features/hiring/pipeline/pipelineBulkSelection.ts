...
export type BulkWhich = "reject" | "outreach";
...
export type BulkResult = {
  ok: number;
  failed: number;
  verb: "moved" | "accepted" | "rejected" | "invited" | "drafted" | "departed";
  reason?: string | null;
  reasonCodes?: string[];
 ...
 *  on any ACTIVE entry. An id with no entry on the board touches nothing. */
...
export function armedBulkConfirm(
  ...
  return armedConfirm(state.confirm, scope, cohortSignature(state.selected, entries, state.confirm.which));
}
...
function withSelection(state: BulkSelectionState, selected: ReadonlySet<string>): BulkSelectionState {
  return { ...state, selected, confirm: bulkConfirmReducer(state.confirm, { type: "selectionChanged" }), result: null };
}

export function bulkSelectionReducer(state: BulkSelectionState, ev: BulkSelectionEvent): BulkSelectionState {
  switch (ev.type) {
    case "reset":
      return { ...state, selected: new Set<string>(), confirm: null, result: null };
    case "toggle": {
    ...
    case "cancel":
      return { ...state, confirm: bulkConfirmReducer(state.confirm, ev) };
    case "fired":
      return { ...state, confirm: bulkConfirmReducer(state.confirm, { type: "fired" }), result: null, busy: ev.busy ?? state.busy };
    case "outreachStarted":
      return { ...state, outreach: { taskId: ev.taskId, cohort: [...ev.cohort] } };
    case "settled":
      return { ...state, selected: new Set(ev.keep), confirm: null, result: ev.result, busy: false };
