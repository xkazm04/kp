...
import { rowLeader } from "@/app/features/hiring/decisions/groupEval/groupEvalHelpers";

const DIR = path.dirname(fileURLToPath(import.meta.url));
const src = readFileSync(path.join(DIR, "GroupEvalComparisonTable.tsx"), "utf8");
// The candidate-header cell (KO pill vs. Lead crown) was split out into its own
// row-building-block module (GroupEvalComparisonRows.tsx) to keep the table
// component under the 200-line cap.
const headerSrc = readFileSync(path.join(DIR, "GroupEvalComparisonRows.tsx"), "utf8");
