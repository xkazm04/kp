...
const src = readFileSync(path.join(path.dirname(fileURLToPath(import.meta.url)), "DevTab.tsx"), "utf8");
// Isolate the approve() function body.
const start = src.indexOf("const approve = async");
assert.ok(start >= 0, "approve() must exist");
const body = src.slice(start, src.indexOf("const selectedCase", start));
