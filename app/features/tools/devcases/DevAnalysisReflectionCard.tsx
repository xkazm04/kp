...
export function DevAnalysisReflectionCard({ result, analysis }: { result: Result; analysis: NeedAnalysis }) {
  const t = useTranslations("devcase.studio.reflection");
  return (
    <div className={`${PANEL} p-4`}>
      <div className="mb-2 flex items-center gap-2">
        <span className="text-meta uppercase tracking-wide text-steel">{t("title")}</span>
        {analysis.trueComplexity ? (
          <span className={`rounded-full px-2 py-0.5 text-micro font-semibold uppercase ${COMPLEXITY[analysis.trueComplexity] ?? "bg-stone-100 text-steel"}`}>
            {t("complexity", { level: analysis.trueComplexity })}
          </span>
        ) : null}
        <span className="ml-auto flex items-center gap-1.5 text-micro uppercase text-steel">
          ...
          <span>{t("confidence", { value: formatFraction(analysis.confidence ?? 0, { label: "confidence" }) })}</span>
        </span>
      </div>
      ...
      {(analysis.statedVsRealGaps ?? []).length > 0 ? (
        <div className="mt-3">
          <p className="text-micro font-semibold uppercase tracking-wide text-coral">{t("gaps")}</p>
      ...
      {(analysis.riskAreas ?? []).length > 0 ? (
        <p className="mt-2 text-micro text-steel">{t("risks", { areas: (analysis.riskAreas ?? []).join(" · ") })}</p>
      ) : null}
    </div>
  );
}
