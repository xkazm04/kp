...
export function OutboxTable({ outbox, state, onResent }: { outbox: OutboxItem[]; state: LoadState; onResent?: () => void }) {
  ...
  // an empty list is ambiguous between "still loading" and "no messages yet" —
  // `state.lastUpdated` disambiguates the two.
  if (outbox.length === 0 && state.lastUpdated == null && !state.failed) {
