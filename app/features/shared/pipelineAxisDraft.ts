// Pure editing model for the board's column axis.
//
// Lives in shared/ because TWO feature groups edit the same axis under the same
// rules: Settings → Hiring (the full composer) and the first-run wizard's
// Pipeline step (app/features/shell/setup/), which offers the same shape on a
// brand-new workspace. One model, one validator — a second copy in the wizard
// would drift from the server contract the moment either side changed.
//
