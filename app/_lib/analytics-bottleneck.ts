...
/** The stage with the highest median days-in-stage, considering only stages
 ...
 *  DETERMINISTIC on ties: equal medians resolve to the lowest stage name. */
export function pickBottleneck(
  ...
  for (const [stage, days] of Object.entries(perStageDays)) {
    ...
    const stageMedian = median(days);
    if (stageMedian === null) continue;
    ...
    if (stageMedian > bestMedian || (stageMedian === bestMedian && best !== null && stage < best.stage)) {
      bestMedian = stageMedian;
