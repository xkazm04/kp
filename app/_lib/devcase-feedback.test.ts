...
test("builds a warm brief with strengths and reframed growth areas", () => {
  const b = buildFeedbackBrief({
    candidateRef: "Alex",
...
test("carries NO adverse / rejection wording — that stays human-gated", () => {
  const b = buildFeedbackBrief({
    candidateRef: "Sam",
    roleTitle: "Data Eng",
    strengths: ["X"],
...
test("an empty evaluation still yields a kind, non-empty note", () => {
  const b = buildFeedbackBrief({ candidateRef: "Pat", roleTitle: null, strengths: [], concerns: [], gaps: [] });
  assert.match(b.body, /appreciated your effort/i);
  assert.doesNotMatch(b.body, NO_ADVERSE);
  assert.doesNotMatch(b.body, /What stood out/); // no empty section header
});

test("blank entries are dropped, not rendered as empty bullets", () => {
  const b = buildFeedbackBrief({ candidateRef: "Lee", strengths: ["Real", "  "], concerns: [""], gaps: [] });
  const bullets = b.body.split("\n").filter((l) => l.startsWith("- "));
  assert.deepEqual(bullets, ["- Real"]);
});

test("falls back to a generic greeting/subject when fields are missing", () => {
  const b = buildFeedbackBrief({ candidateRef: "", strengths: [], concerns: [], gaps: [] });
