...
const { withLlmRequestId, withLlmRequestIdIfUnset, currentLlmRequestId } = await import(
  "./llm-request-context.ts"
);
...
test("withLlmRequestIdIfUnset keeps the id an outer scope already opened", async () => {
  // A companion turn generated by the digest TASK runs inside the runner's
  // scope. Stamping its own id there would shadow the task id and break the
  // Activity join for the run the operator can actually open.
  const got = await withLlmRequestId("t_task", () =>
    withLlmRequestIdIfUnset("companion:cthread-1:cturn-1", () => spawnAfterAwait()),
  );
  assert.equal(got, "t_task");
});

test("withLlmRequestIdIfUnset stamps its own id when nothing is open", async () => {
  // The interactive route: no task, so the turn names itself.
  const got = await withLlmRequestIdIfUnset("companion:cthread-1:cturn-1", () => spawnAfterAwait());
