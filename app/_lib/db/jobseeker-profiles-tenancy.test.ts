...
// Tenant scope — source guard for the jobseeker_profiles table (same shape as
...
const src = readFileSync(path.join(path.dirname(fileURLToPath(import.meta.url)), "jobseeker-profiles.ts"), "utf8");
...
function isInsert(sql: string): boolean {
  return /\binsert\s+(or\s+\w+\s+)?into\s+jobseeker_profiles\b/i.test(sql);
}
...
test("every SELECT/UPDATE/DELETE/INSERT on the jobseeker_profiles table carries workspace_id", () => {
  ...
  for (const sql of touching) {
    ...
    assert.ok(
      required.test(sql),
      `a jobseeker_profiles query is NOT workspace-scoped (a bound workspace_id = ? predicate is required; a SELECT-list mention is not scoping):\n${sql.trim().slice(0, 200)}`
    );
  }
});
...
test("the guard rejects a workspace_id that is merely SELECTED, never filtered", () => {
  const leak = `SELECT id, workspace_id FROM jobseeker_profiles ORDER BY created_at DESC LIMIT ?`;
