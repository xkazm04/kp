...
// Tenant scope — source guard for the jobseeker_postings table (same shape as
...
const src = readFileSync(path.join(path.dirname(fileURLToPath(import.meta.url)), "jobseeker-postings.ts"), "utf8");
...
function isInsert(sql: string): boolean {
  return /\binsert\s+(or\s+\w+\s+)?into\s+jobseeker_postings\b/i.test(sql);
}

test("every SELECT/UPDATE/DELETE/INSERT on the jobseeker_postings table carries workspace_id", () => {
  const touching = sqlBlocks.filter((s) => /\b(from|into|update|delete\s+from)\s+jobseeker_postings\b/i.test(s));
  assert.ok(touching.length >= 13, `expected >=13 jobseeker_postings queries, found ${touching.length}`);
  for (const sql of touching) {
    const required = isInsert(sql) ? STAMPS_SCOPE : BOUND_SCOPE;
    assert.ok(
      required.test(sql),
      `a jobseeker_postings query is NOT workspace-scoped (a bound workspace_id = ? predicate is required; a SELECT-list mention is not scoping):\n${sql.trim().slice(0, 200)}`
    );
  }
});
...
test("the guard rejects a workspace_id that is merely SELECTED, never filtered", () => {
  const leak = `SELECT id, workspace_id FROM jobseeker_postings ORDER BY created_at DESC LIMIT ?`;
