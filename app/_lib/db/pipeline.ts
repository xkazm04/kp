...
export const BOARD_ENTRY_FIELDS = [
  ...
  "sourceCampaign",
  "sourceVariant",
  // ADR-0009 — the board renders ONE list holding both populations, so the row
...
type PipelineRow = {
  ...
  // ADR-0009 — slate columns. Optional here for the same reason as the others:
...
function rowToEntry(r: PipelineRow): PipelineEntry {
  return {
    ...
    // ADR-0009 — narrowed at the read boundary (the column is free-form TEXT and
...
export type CreatePipelineInput = {
  ...
  workspaceId?: string;
  // ADR-0009 — which population this candidate belongs to. Omitted by every
