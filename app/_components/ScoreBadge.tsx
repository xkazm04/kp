...
export function ScoreBadge({ score }: { score: number | null }) {
  ...
  if (tone === "null") {
    return <span role="img" aria-label={t("unscored")} className={`${base} ${TONE_CLASS.null}`}>—</span>;
  }
  ...
  const displayed = Math.round(clampPercent(score as number));
  return <span role="img" aria-label={t("scoreAria", { score: displayed })} className={`${base} ${STAMP} ${TONE_CLASS[tone]}`}>{displayed}</span>;
}
