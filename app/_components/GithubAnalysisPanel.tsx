  ...
  return (
    <section className="animate-fade-in rounded-lg border border-stone-200 bg-white p-5 shadow-panel">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        {status === "loading" ? (
          <div className="inline-flex items-center gap-2 rounded-md bg-paper px-3 py-2 text-sm font-medium text-steel">
      ...
      {status === "error" ? <p className="mt-4 rounded-md bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[380px_1fr]">
      <div className="space-y-5">
        <div className="rounded-lg border border-stone-200 bg-paper p-4">
          ...
          <p className="mt-3 text-sm leading-6 text-ink">{analysis.summary}</p>
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-lg border border-stone-200 bg-white p-4">
          ...
          <p className="mt-3 text-sm leading-6 text-ink">{analysis.jobFitSignals.complexityAssessment}</p>
        </div>
        ...
        {repositories.map((repo) => (
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex shrink-0 gap-2 text-xs text-steel">
            ...
            {repo.description ? (
              <p className="mt-2 line-clamp-2 text-xs leading-5 text-ink">{repo.description}</p>
            ) : null}
...
function CodeReviewBlock({
  ...
  return (
    <div className="rounded-lg border border-stone-200 bg-white p-4">
      ...
      {review.error ? (
        <p className="mt-2 rounded-md bg-red-50 p-2 text-xs text-red-700">{review.error}</p>
      ) : null}
  ...
  return (
    <div className={`rounded-md ${accent} p-3`}>
      ...
      {items.length ? (
        ...
        <p className="mt-2 text-xs text-steel">None detected.</p>
      )}
    </div>
  );
}
