...
import { TTS_UNAVAILABLE_CODE, readAutoSpeak, shouldAutoSpeak, storeAutoSpeak } from "./studioVoiceIo";
import { useUnavailableLatch } from "./useUnavailableLatch";

/*
 * A studio composer's OUTPUT pipeline — an agent turn, read aloud. The other
 ...
 *      is `shouldAutoSpeak` in studioVoiceIo.ts, where it is testable. The
 *      preference is stored under the CONSUMER'S key (`autoSpeakStorageKey`).
 ...
 *
 * NO PROBE ON MOUNT. `GET /api/tts` probes every configured provider, and a
 * studio is mounted for the whole session while most sessions never press
...
export function useStudioSpeech({
  speakText,
  lang,
  sessionKey = null,
  autoSpeakStorageKey,
  ...
  lang: string;
  /** The session this text belongs to. Switching sessions re-primes and
  ...
  /** localStorage key for the auto-speak preference — the consumer's. */
  ...
  // Read lazily, the SSR-safe shape the desk's stored zones use (studioZones.ts):
  // the reader is `window`-guarded, so a server render takes the default and the
  // browser takes the stored value on its first paint rather than flipping the
  // control a tick later.
  const [autoSpeak, setAutoSpeakState] = useState<boolean>(() => readAutoSpeak(autoSpeakStorageKey));
  const setAutoSpeak = useCallback(
    (value: boolean) => {
      setAutoSpeakState(value);
      storeAutoSpeak(autoSpeakStorageKey, value);
    },
    [autoSpeakStorageKey]
  );
  ...
  useEffect(() => {
    if (sessionRef.current !== sessionKey) {
      ...
      // the session the reader just left is audio with no visible source.
      ttsStop();
    }
    ...
    // when the reader comes back would be an answer arriving minutes late.
