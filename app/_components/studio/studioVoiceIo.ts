// The decisions behind a studio composer's voice pair — PURE, so the two hooks
// beside it (useStudioDictation, useStudioSpeech) stay thin wrappers over the
// portable packages and every rule that could be got wrong is provable under
// `node --test` with no DOM. (Written for the intake studio as `intakeVoiceIo.ts`,
// whose tests still pin it through that path; the kit is where it lives now.)
//
 ...
 *
 * PRIMING is the subtle half. Opening a stored session hands this surface the
 * last agent turn immediately, and that turn may be a week old — speaking it
 * would be the app talking at a reader who has just arrived. So the first
 * evaluation of a mount records what it saw and says nothing; only a CHANGE
 * after that is an arrival. The same rule makes switching the preference on
 * silent: priming has already happened, so nothing speaks until the agent next
 * answers.
 *
 ...
 * agent ever sees it. The reader may also have typed half a sentence before
 * pressing the mic, and a transcript that clobbered it would lose words the app
 * never had to lose.
 *
 * The spacing rules are small and all of them are "do not make the reader fix
 * punctuation by hand":
 ...
 *    so localStorage — under the CONSUMER'S key, passed in, so two studios on one
 *    machine keep two preferences. ── */
...
export function readAutoSpeak(storageKey: string): boolean {
  if (typeof window === "undefined") return DEFAULT_AUTO_SPEAK;
  try {
    return parseAutoSpeak(window.localStorage.getItem(storageKey));
  } catch {
...
export function storeAutoSpeak(storageKey: string, value: boolean): void {
  try {
