...
// THE PLANE, and the zone that lives on it.
//
// A nested desk puts three rounded, bordered cards inside another rounded
// bordered card. The studio keeps the zones and the information and removes the
// boxes. There is ONE surface; a zone is separated from its neighbour by a
// hairline, and its hierarchy comes from type and space — an uppercase mark, a
// bare tabular numeral, a band of air — not from a radius and a shadow.
//
...
// so a reader looking at the plane sees the plane and not three chrome buttons.
//
...
//
// Strings come from the consumer's namespace: `<ns>.columns.hide` / `.show`.

/** House ease (docs/design/README.md · AnalyzeWorkspace's segmented standard). */
export const STUDIO_EASE = [0.16, 1, 0.3, 1] as const;
export const STUDIO_SPRING = { type: "spring" as const, stiffness: 420, damping: 34 };
...
export function StudioZone({
  ...
  zoneKey: string;
  label: string;
  /** A bare numeral, or a one-glyph state. Never a pill. */
  count: string;
  /** What the numeral means — carried in the fold control's tooltip and read out. */
  countHint: string;
  open: boolean;
  /** False on the last open zone and on a pinned one: the desk never folds to nothing. */
  canFold: boolean;
  onToggle: () => void;
  ns: string;
  ...
  return (
    <motion.section
      layout={!reduced}
      transition={{ layout: { duration: reduced ? 0 : 0.28, ease: STUDIO_EASE } }}
    ...
    >
      {open ? (
        <>
          <div className="sticky top-0 z-10 flex shrink-0 items-center justify-between gap-2 bg-white pb-2">
            ...
            <span className="opacity-0 transition-opacity group-focus-within/zone:opacity-100 group-hover/zone:opacity-100">
              <IconAction
                icon={Minimize2}
                label={t("columns.hide", { column: label })}
                hint={countHint}
                side="bottom"
                size={14}
                disabled={!canFold}
        ...
        <button
          ...
          onClick={onToggle}
          aria-expanded={false}
          aria-label={t("columns.show", { column: label })}
          title={countHint}
