...
import { STT_UNAVAILABLE_CODE } from "./studioVoiceIo";
import { useUnavailableLatch } from "./useUnavailableLatch";

/*
 * A studio composer's INPUT pipeline — capture → transcript → an editable
 * field. One of the two halves of the voice pair; it shares nothing with
 * useStudioSpeech but a row in the composer, which is the point (the registry's
 ...
 *      studioVoiceIo.latchUnavailable). The package already latches its own
 ...
 *      speech is the only tool a reader has to debug their own audio.
 *
