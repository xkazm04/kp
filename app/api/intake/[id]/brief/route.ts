...
// definition — the client diffs and claims provenance; the server enforces
// SHAPE (closed vocabularies, 0..1 clamps, caps — brief-edit.ts) and the
// lifecycle: a PROMOTED session's brief is frozen (the JD exists; 409 with a
// clear note — re-promoting an edited brief is future work, documented in the
// feature doc). The transcript is never touched by an edit — the dialog
// record stays honest.
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const brief = sanitizeEditedBrief(body.brief);
