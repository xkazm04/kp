  ...
  try {
    ...
    if (coerced.payload.orgId !== orgId) {
      return NextResponse.json(
        {
          error:
            "This backup belongs to a different organization. A backup is restored into the organization it came from — moving an organization between deployments is not supported.",
        },
        { status: 409 }
      );
    }
    ...
    if (plan.totalExisting > 0 && !body.replace) {
      return NextResponse.json(
        {
          error: "Restoring replaces the organization's current data — confirm replace to proceed.",
