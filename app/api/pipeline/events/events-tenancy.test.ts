...
// backs three reads — the newest-page feed, the since-cursor catch-up, and the
// per-entry drawer history. Before the fix the route omitted the workspace
// argument each DB function accepts, so it fell to DEFAULT_WORKSPACE_ID and a
// second tenant's feed + drawer read the wrong team's audit trail.
//
...
test("the events route threads the caller's workspace into ALL THREE reads (bug-fix guard)", () => {
  ...
  // ...and passes it (as `ws`) to each of the three reads — a bare call with no
  // workspace argument would silently fall to DEFAULT_WORKSPACE_ID again.
  assert.match(src, /listPipelineEventsForEntry\([^)]*\bws\b[^)]*\)/, "per-entry read must be workspace-scoped");
