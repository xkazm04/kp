...
- **4-locale message parity.** Every key added to `messages/en.json` must land
  in `cs`/`de`/`fr` in the same change — `npm run i18n:check` gates it, and
  next-intl keys are TYPED: a `t("missing.key")` is a `tsc` error, so an
  incomplete catalog breaks `npm run typecheck` for everyone.
- **No raw hex outside `app/landing/`.** `npm run design:check` enforces the
  token rule from the design-system section below.
