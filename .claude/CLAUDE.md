...
- **Design tokens:** see the design-system section above. TWO gates hold it, and
  ...
  rule's options, and a later `no-restricted-syntax` block shadowed it. Every
  block's options are now built by `restrict()` from declared selector sets; if
  you add a block there, build it the same way and include every set its files
  owe. `app/lint-selector-coverage.test.ts` asks eslint what it actually
  resolved per layer and fails if a set went missing.
