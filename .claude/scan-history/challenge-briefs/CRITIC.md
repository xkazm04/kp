# Challenge critic brief — /scan-sweep 3.5.0 `--challenge`, kp (reusable; <RUN> = the run directory named in your prompt)
...
Cards: every `<RUN>/cards/*.json` (each a JSON
array of two cards).
...
3. **Never re-propose**: does it restate a title in
   ...
   is NOT resolved in `low-risk-drain-2026-09-22*-results.jsonl`? Or restate a card in a PRIOR challenge deck
   (`.claude/scan-history/challenge-*/cards/*.json` outside this run)? Adopting an
