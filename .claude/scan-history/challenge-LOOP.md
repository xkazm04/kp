...
- OWED (test harness): scripts/run-unit-tests.mjs re-runs a failed file by its path as a GLOB, so a bracketed route path (app/api/comms/[id]/...) matches 0 tests on the re-run and a GENUINE failure is labelled FLAKE (seen r09 resend follow-up). A FLAKE still fails the build, but the verdict lies; escape [ ] (or pass the path literally) on the re-run. Touches the flake-policy gate (test:flake) — owner review.
- GAP (pre-existing design, surfaced by r09): every homework-arrival refusal (unaddressable, suppressed, billing, no_case, intake_stopped, error) is LOG-ONLY — the candidate stays in the Homework column with no approval/flag saying why no assignment went out (stage-hooks-homework.ts refuse(); the interview hook uses failOpenToTheHumanQueue instead). The code names the fix: a dedicated approval kind for "waiting for an assignment". Candidate for a later challenge card.
...
- OWED at loop end: keyless e2e subset (e2e/landing.spec.ts: r09 spotlight walk + reduced-motion about art — written, never run; shell.spec + public-pages.spec after r07 jd-public-detail/B hreflang change, and a manual look at /jds/<slug>?lang=cs with a stored translation; profile-builder.spec was edited by r04 profile-editor/B; token-doors-axe by r02 llm-api/A) against a KP_EMPTY=1 prod build.
...
| challenge-r09 | 8 | 0 | done — 16/16 landed + 5 follow-ups; flawless 12/16 (8 strict); critic 3.88/4.75/4.25; 5 coordinator fixes (3 settles, stage-hooks tsc fix 7769a80b1, readiness lens waiver); unit fully green twice; owner notes in REPORT |
