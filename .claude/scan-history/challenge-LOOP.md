...
- OWED at loop end: browser pass (both themes) over the new UI surfaces the builders could not open — incl. r06 offer-letter preview pane, metric-pack preview, agent roster next move, devcase in-flight close warning + intake-closed banner, bulk-move preview, status next-action card; Analyze drag-and-drop (r03 cv-analyze/A), About transport, schedule pending cards, routing chips, promote verdict, receiver editor, SLA editor, glyph empty states.
...
| challenge-r07 | 8 | 4 | critic done (ambition 4.06 / grounding 4.56 / falsifiability 4.06; 0 premise-false, 0 void; 4 build, 12 revise — devcase-orch/A close half voided by 5075b3b08); builds after r06 closes, waves per critic.json `after` |
