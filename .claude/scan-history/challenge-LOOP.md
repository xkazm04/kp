...
- OWED at loop end: browser pass (both themes) over the new UI surfaces the builders could not open — incl. r07 decision ack editor, hire-rating queue, interview coverage cells, models routing pick board, setup finish receipt, dev-case outcome chips, Roles desk window; r06 offer-letter preview pane, metric-pack preview, agent roster next move, devcase in-flight close warning + intake-closed banner, bulk-move preview, status next-action card; Analyze drag-and-drop (r03 cv-analyze/A), About transport, schedule pending cards, routing chips, promote verdict, receiver editor, SLA editor, glyph empty states.
...
| challenge-r08 | 8 | 0 | critic done (4.13/3.88/4.19; 0 premise-false, 0 void; 3 build, 13 revise — grounding dipped: line cites off by 100-200 in archetypes/A, past-EOF in taxonomy/A); page.tsx already red for shell A/B (budget raise owed); waves per critic.json; builds after r07 |
