...
- **Prod-build smoke beside the operator's dev server: build AND start with `KP_EMPTY=1`** (distDir `.next-empty`, fixture seeding off - seed through the store modules with tsx first). `next dev` boot cleans `.next`, so a `next start` on the shared dir answers 500 for every chunk the moment the operator restarts (2026-09-16). Headless recipe: `KP_EMPTY=1 KP_DB_PATH=data/kp-smoke.sqlite KP_ALLOW_OPEN=1 npx next start --port 3111`, Playwright scripts copied to the repo root for module resolution, `KP_E2E_BASE_URL` for the declared specs; stop with `taskkill //PID` from `netstat -ano` (no pkill in Git Bash). Turbopack refuses a junctioned node_modules for `next build`, so the worktree recipe is typecheck-only.
...
## User taste
- 2026-09-16 (round 27): when the operator says a surface is in 'terrible shape', the winning move is a CATALOGUE of the house techniques + a per-page AUDIT against it, then inheritance lots - no slate gate needed, the order was explicit; three builder deviations argued on doctrine were all right.
- 2026-09-16 (jobseeker round): accepts a full 5-direction slate when every item is evidence-cited and bugfixes stand alone; steers a freshly shipped feature straight into /perfect the same day - a same-day round on a just-shipped module is welcome, not churn.
...
- 2026-09-25 (round 28, /me career-change arc) **A live run with the operator's real data found six defects that two waves of green gates could not.** EURES reads a keyword as ANY of its words, its API had moved to `locationMap`, an API page outgrew the fetch cap, and an AI draft's paraphrase reached a document that promises the seeker's own words - every one passed unit tests because the fixtures encoded the old assumption. Rule for engine rounds with external feeds: end the round with ONE live end-to-end run on real inputs (scratch DB) BEFORE declaring done; budget a fix pass for it.
- 2026-09-25 **Class-C catalogs worked cleanly at scale**: builders reported 4-locale fragments, the Director applied 66 adds and 42 dead-entry deletions with a surgical raw-text inserter/deleter, staged as `git show HEAD:` + only these keys via hash-object, so a sibling session's uncommitted catalog lines never rode along. No duplicate-key or race incidents (contrast waves 7-8).
- 2026-09-25 **A dev server killed mid-compile leaves a stale Turbopack cache that 404s whole route folders** (routes in the manifest, handler never reached, HTML not-found). Clear `<distDir>/dev` before restarting; do not debug the app first.
- (2026-09-02/03, waves 7-8) Two builders died to a network outage (ENOTFOUND) mid-direction. Salvage that worked: wip snapshots per lot with `--only --no-verify` + the exemption trailer (NOTE: `git add <new file>` first - `--only <dir>` cannot see untracked files), then `SendMessage` to the dead agent id resumes it from its own transcript - far cheaper than a fresh brief, and both finished their lots. A builder briefly `git stash`ed the shared catalogs to work around a sibling's uncommitted keys; nothing was lost, but the brief now spells out the no-stash alternative (splice HEAD + your keys + the sibling's lines, line-surgical).
- (2026-09-03, wave 8 gate) A cross-lot regression surfaced ONLY at the integrated gate: lot V2 added a new pipeline event kind and the decision-attribution guard test (writers vs DECISION_META) caught it unmapped. Neither builder's scoped test run could see it. RULE kept: the Director runs the full unit suite on the quiet tree before every merge, and fixes small cross-lot fallout itself rather than re-dispatching.
- (2026-09-03) `i18n:check` could not see a key repeated inside one catalog object (JSON.parse keeps the last silently) - wave 7's three-lot catalog splicing landed five keys twice in all four catalogs and the gate said OK. Fixed in the gate (text walk, dotted path in the message). Lesson for the method: a gate that runs on the PARSED artifact is blind to duplication in the SOURCE; when several lots splice one file, add a source-level check.
- 2026-09-02 (mkdol machine, rounds 1-2 after the incremental rescan) **The context map had 326 unmapped files (12%) two weeks after a "no remainder" rescan.** An incremental rescan (drop stale, place new by sibling-source -> dir affinity -> keyword tiebreak, seed contexts for homeless clusters) took one script and one commit; do it at Phase 0 whenever `git ls-files` minus the map is non-trivial, never wait for the app to rescan.
- 2026-09-02 **`core.autocrlf=true` makes a CRLF checkout fail any source-guard test whose marker contains `
`** (the repo-scan limiter contract was red on every Windows checkout since 2026-08-23 and green on CI). Normalise line endings in the test's reader; a baseline gate run BEFORE the wave is what made this attributable rather than a builder's fault.
- 2026-09-02 **All three wave-2 builders died to one session limit within a minute of each other.** Salvage rule held again: one direction was already committed, two were 80% done on disk. Three `wip(...)` snapshots with `--only <write set> --no-verify` + a `Commit-convention-exemption` trailer, then fresh builders "continue from <sha>". Attribute shared Class-B files (core.ts, catalogs) to a lot BEFORE snapshotting - here each was single-lot, so no mixed snapshot was needed.
- 2026-09-02 **Operator-delegate gate.** The owner asked for autonomous cycles; the Director gated its own slates (taste filter, engine depth, bugfixes standalone) and left three product calls explicitly undecided (hard timebox enforcement, judge independence as a recommendation term, the Worker `/in/<token>` limiter). Record every un-taken decision in the session note so the owner can take it in one keystroke later.
- 2026-09-02 (waves 2-4) **Class-B splice beats whole-file `--only`.** Two builders' catalog keys were swept into a sibling's commit by whole-file `--only`; lot D found the fix: splice your block onto `git show HEAD:<file>`, commit, then write the siblings' working-tree content back. Now in every brief. Also: after an `index.lock` retry, RE-CHECK the commit's `--stat` - a retry once silently dropped the catalog paths from the pathspec (lot J had to land them in a second commit).
- 2026-09-02 **Additive package codes need a Director relay, not a shared write set.** Lots S and T added `rate_limited`/`too_long` error codes to the voice packages; the host routes belonged to lot V. One SendMessage with the recommended HTTP mappings let V map both before its own gate - no overlap, no re-brief. Prefer "package lot ADDS, host lot MAPS, Director relays" whenever a wave spans a package and its consumer.
- 2026-09-02 **A builder's test can be right and nondeterministic at once.** The Worker test signed one malformed body three times inside one millisecond; the nonce guard correctly answered 409. The rule was right, the fixture was wrong - fix the fixture, keep the rule, and say so in the commit.
- 2026-09-02 (waves 5-6) **Never gate a wave on the shared tree after the next wave is dispatched.** Wave 6's i18n:check went red on the worktree because wave-7 builders had already added registry codes without their catalog keys. The wave's own commits were green. Rule: run the integration gate BEFORE dispatching the next wave, or verify at the merge commit on main (where no builder is editing). Also: a builder that flips a file's line endings (autocrlf) makes the next merge a whole-file conflict - resolve by taking the wave file and re-applying the Director hunk, never by discarding either.
- 2026-09-02 (wave 7) **A Class-B splice is only safe inside ONE shell invocation.** `git show HEAD:<catalog>` + insert + write + `git commit --only` across several tool rounds lost the race three times in one wave (a registry block committed empty twice; one commit deleted six of a sibling's keys; a mid-write state left all four catalogs unparseable at HEAD). The rule now in every brief: read-splice-write-commit in a single shell command, then `git show HEAD:messages/en.json | JSON.parse` and `--stat` before moving on. Better still: give catalog-heavy directions to ONE lot per wave, or have builders REPORT their keys and let the Director apply them once at quiescence (the Class-C shape the method already names).
- 2026-09-02 **Worktree-only unit failures exist:** `decisions-auth` and `pipeline-routes` 500 through the junctioned node_modules (NextResponse dual-module) and pass on main. Re-run the failing files on main before blaming a wave; say so in every brief so builders stop investigating it.
...
- 2026-09-04 (rounds 39-41, waves 39d-40d) **"Exists only if it renders" is the highest-yield review question for lib lots.** Four of nine builders this stretch shipped a flag, count or disclosure the route answered but no surface rendered (JW truncation, SC confirmation zone, GE consent/degraded, SP's two unjudged doors). Each was closed by ONE SendMessage extending the write set to the consumer - never a re-brief. Ask it of every report before reading the diff.
- 2026-09-04 **A doc path in a brief is a claim.** Three briefs named docs that do not exist (`persistence.md`, `decisions/README.md`) or the wrong file for a hash; builders coped, but each cost a DECISION NEEDED round. Brief r23: the builder resolves the mapped doc from `feature-doc-map.json`; the Director greps every `docs/` path in a lot brief against the tree before dispatch (one loop, seconds).
- 2026-09-04 **Cross-lot fallout at the integrated gate is now the norm, and small.** Five gates this stretch: three reds total, all fixture-level (a guard demanding a literal the lot had aliased; a tenancy fixture the new schema refuses; an unmapped event kind). The Director fixes in minutes; a re-dispatch would cost an hour. Keep the rule: Director fixes fallout under ~20 lines itself, records the commit in every KEEP line.
- 2026-09-04 **Plan waves by write-set collisions before the round starts, not lot by lot.** Round 41's nine remaining lots have three pairwise collisions (instrumentation-node.ts, db/core.ts, ci.yml); one grep over the write sets produced the 41b/41c/41d plan in one pass, where wave-40 planning found the db/core.ts collision only at dispatch time.

- 2026-09-05 (round 41, waves 41b-41d) **A spec never run rots silently, and "enrol in CI" without one live run is a red build waiting.** Lot E2 enrolled seven dormant keyless specs and could execute none (no server; the worktree cannot build). The Director booted main's dev server on :3111 with an isolated `KP_DB_PATH` in the scratchpad: 5 of 7 specs were broken against today's app (tables another session reworked the day before), one assumed a populated ledger, one found a real AA contrast defect on the primary button. Rule: any e2e lot gets a Director-provided server (`KP_E2E_BASE_URL`) in its brief, and a spec that needs an APP change is dropped from the list, never holdout-listed.
...
- 2026-09-05 (round 43) **A base that moves under the wave is one content merge, then the gate again on main.** A sibling session landed three registry-map commits mid-wave with no file overlap; `--ff-only` refused, `--no-ff` merged clean, and typecheck + docs + unit were re-run on the merge commit before the push. Check `comm -12` of the two changed-file sets first: no overlap means no conflict to reason about.
- 2026-09-05 (round 43) **Package-lot ADDS, host-lot MAPS, Director relays - worked again across contexts.** Lot EG added an `EdgeErrorKind` and its catalog keys; the card file belonged to the later lot EW, which got the one-line label as the first item of its brief. Zero write-set overlap, zero re-brief. Also: a delta re-scout found a prior direction whose payoff never delivered (the redaction sweep named a field no model has) - the scout prompt "did it work, not did it merge" is what surfaces these.
- 2026-09-05 (round 44) **The Director's own patches go through a script FILE, never a bash heredoc.** Two heredoc patches in one day silently mangled backslashes (`\b` became 0x08; `\s` in a regex tail failed its anchor before writing anything). A Python file written with the Write tool and run by path preserves bytes; the heredoc does not. Also: a delta re-scout the day AFTER a wave is the cheapest place to catch the wave's own overstatement (the bench measured reuse the morning after coalescing landed).
- 2026-09-05 (round 44) **Sequential lots over a shared file beat one 4-direction lot.** Sim directions 1+3 and 2+4 shared the reset route and the store; splitting them into SM1 (44a) and SM2 (44b, briefed on SM1's landed seam) kept every lot at <=2 directions and the seam explicit. SM2 still hit the one export it needed in SM1's file - a Director one-liner at quiescence, not a re-brief.
- 2026-09-05 (round 45) **Fold the Director's own drift into the next lot's brief as item 0.** Two Director commits from the morning (the UNAVAILABLE codes) left doc and test drift the evening scouts found; giving each affected lot the drift as its first item cost nothing and kept the fix beside the code it describes. Also: a lot that edits the gates that judge its own push must run the constitution check over its own range before reporting (GT1 did; 0 blocking).
- 2026-09-05 (round 45) **Smoke debt is now explicit.** Three rounds shipped browser-facing surfaces no one drove (dock focus/reconnect, voice waiting/unsupported-language notes, the mic, the sim console after a reload). The `next:` pointer names a `/perfect smoke` pass owed before round 47; a smoke pass every ~3 waves is the overlay rule and it slipped.
- 2026-09-05 (round 45, merge) **A sibling can switch the MAIN CHECKOUT to its own branch mid-session; check `git status -sb`'s first line before every merge, commit or push there.** A /tiger session moved the main checkout to `tiger/automation-engine-fixes`; the Director's `--ff-only` merge failed against that HEAD (the pipe hid the exit code, so the wrap ran with the wrong sha) and one Director commit landed on the sibling's branch (removed with `update-ref` to its parent, verified one-commit-of-mine). Land a gated wave into `main` WITHOUT switching the shared checkout: `git push . <wave>:main` (runs the pre-push hook against the current checkout - fine only when that checkout is clean) or `git update-ref refs/heads/main <wave-tip> <old-main>` after the gates ran on the wave tree; push origin FROM THE WORKTREE so the hook gates the wave tree, never a sibling's dirty one. Never pipe a merge into `tail`.
- 2026-09-16 (round 25, jobseeker) **A just-shipped module is the richest slate source**: the operator's two same-day reports plus one Explore scout yielded 5/5 accepted with zero manufactured findings. **Foreign WIP live => commit onto main with `--only`, no wave branch** - worked; the only cost was catalog keys riding in sibling commits (whole-file `--only`), verified present at HEAD. **A forced-failure live check is cheap and decisive**: Playwright `page.route()` returning an HTML 404 / aborting one API proved the failure-vs-empty split in both themes in one run - use it for every async-state direction. **The recipes-literals ratchet reports OTHER sessions' dirty files** - grep the finding list for your own paths before blaming a lot.
- 2026-09-16 (round 26, jobseeker, same session) **A delta re-scout on a same-day module is worth a full slate**: 'near-polished, 5 residuals' still yielded 5/5 accepted because 3 residuals were engine-level (a NULL-trap skip predicate, a period rule split across Python and TS, taxonomy role words). **Cross-toolchain directions need a named TS owner**: the salary bugfix was briefed to the Python lot and its TS half fell to the Director - name both halves in one direction next time. **Seed the throwaway DB through the store modules (tsx + KP_DB_PATH, before the server starts)**: the anchor/badge/divider/applied-date checks were only observable with data; a forced-failure route intercept covers the other half. **Whole-file `--only` on catalogs keeps sweeping the foreign session's in-flight keys** - the disclosure in the commit body is the mitigation the method allows; the repo's own law should ask sessions to splice catalog blocks.
- 2026-09-16 (round 27, jobseeker UX) **A first live pass catches what no gate can**: column allocation (FIT starving ROLE, chips stacking) passed eslint/th-scope/recipes/design and failed the eye instantly - budget one redo round after every UI lot. **Gate on a clean worktree when the shared tree is red for foreign reasons**, and if HEAD is inconsistent because whole-file catalog staging swept a sibling's in-flight namespace move, sweep their tree once it is green rather than leave main unpushable. **Junction recipe on Windows**: `New-Item -ItemType Junction` via PowerShell (cmd mklink choked on Git-Bash paths); delete the junction with `(Get-Item -Force).Delete()` BEFORE `git worktree remove`. **Seed through the store modules with tsx before `next start`** remains the only way to see data-dependent surfaces (badge, divider, applied date).
- 2026-09-16 (smoke over /me) **The smoke found what three rounds of live passes could not: a production-only boot-order defect** (module evaluated twice across the instrumentation and route chunks) - because the live passes SEEDED the DB and never pressed Scan. A smoke must drive the one action that spawns work, not only the surfaces that render it. **Log incrementally in headless scripts** (console.log per step) - two runs lost every finding to a late crash. **A sibling's commit-all sweep can carry your in-flight fix into THEIR feature commit**; record the SHA where the fix actually lives rather than re-committing.
