...
# Extract a shared useAddToPipeline hook
...
- **Generated**: 6/3/2026, 10:50:07 AM

## Description
RecruiterCandidates.tsx and RediscoverPanel.tsx each hand-roll an identical optimistic add-to-pipeline flow: POST /api/pipeline with the same body shape (stage Screened), plus added/adding/failed state, an aria-live announce string, and retry handling. They have even diverged in state representation (Set+Map in one, Record<string,boolean> in the other). Extract one useAddToPipeline(jobId, jobTitle) hook returning { add, added, adding, error, announce } and have both components consume it.

## Reasoning
Two copies of the same side-effecting flow drift independently (they already have), doubling the surface for accessibility and error-handling bugs. A single tested hook collapses the duplication and lets the other pipeline-add call sites (CandidateDrawer, Results, DecisionsTab) adopt it later.
