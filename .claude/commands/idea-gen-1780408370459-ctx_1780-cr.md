...
## Target Context
- Context ID: ctx_1780321513757_n62ur2k
- Context Name: Dev Case Python Engine
...
**Context Description**:
Backend intelligence for developer cases — design cases, source and analyze repos, evaluate submissions against scenarios, run lifecycle audits and reflection, with shared dev-case models.
...
Found 19 active idea(s) (19 total, excluding rejected/implemented for brevity):

### Pending Ideas (16)
1. **Interviewer debrief copilot from the evaluation** (user_benefit)
   - Generate a structured live-interview debrief from each CaseEvaluation: targeted follow-up questions derived from concerns, low dimension_scores, and u...
2. **Transparent, respectful candidate results report** (ui)
   - Build a candidate-facing report that renders the CaseEvaluation as a dimension radar (using the self-describing DimensionScore rows so nothing is hard...
3. **Audit-ready fairness and bias certification report** (user_benefit)
   - Turn the existing fairness gate in submission_eval.py (no invented over-reliance, verification rewarded, AI use not penalised) plus the discrimination...
4. **One-click case generation from a real repo URL** (functionality)
   - Wire a self-serve flow where a hiring manager pastes a GitHub URL and gets a probe-laden CaseScenario in minutes: it reuses the existing RepoSnapshot ...
5. **Portable candidate capability passport** (user_benefit)
   - Let a candidate (with consent) own and reuse the capability profile produced by evaluate.py — the dimension radar, commit-reflection narrative, and pu...
6. **Close the loop with post-hire outcome calibration** (functionality)
   - Capture lightweight post-hire outcomes (ramp speed, probation pass, 6-month rating) keyed to each Submission/CaseEvaluation, then back-test which of t...
7. **Capability benchmark network from eval data** (functionality)
   - Persist every CaseEvaluation's dimension_scores and TransferAssessment by role_family + seniority + domain, then compute rolling percentile bands (e.g...
8. **Disambiguate the role languages field (spoken vs code)** (maintenance)
   - RoleSpec.languages is filled with English by the deterministic design_role fallback (a spoken language) but the LLM prompt can return programming lang...
9. **Rename or document the repoSeed misnomer field** (maintenance)
   - CaseScenario.repo_seed is documented in the design.py prompt as named repoSeed for legacy reasons only and actually holds domain-neutral starting mate...
10. **Enforce devcase pipeline-stage preconditions explicitly** (maintenance)
   - The artifact order DevNeed to NeedAnalysis to Case and Role to Submission to CaseEvaluation to TransferAssessment is documented only in the models.py ...
11. **Surface why an LLM step fell back to deterministic** (functionality)
   - Every _generate helper in analyze.py, design.py, evaluate.py and reflect.py swallows all exceptions with a bare except Exception and returns the deter...
12. **Centralize scattered devcase string enums in one source** (maintenance)
   - Valid values for fields like CoverProbe.kind, seniority, trueComplexity and iterationPattern exist only in inline comments in models.py and are re-lis...
13. **A typed DevCaseLifecycle orchestrator with provenance** (functionality)
   - The canonical chain documented in models.py (DevNeed -> NeedAnalysis -> RoleSpec+CaseScenario -> Submission -> reflection/tooling -> CaseEvaluation ->...
14. **Collapse the two eval harnesses into one EvalHarness** (maintenance)
   - lifecycle_eval.py and submission_eval.py independently reimplement the identical harness: a Row dataclass with a reliable property, a ThreadPoolExecut...
15. **Let Pydantic models own LLM coercion and defaults** (code_quality)
   - models.py defines rich artifacts (NeedAnalysis, RoleSpec, CaseScenario, CaseEvaluation, TransferAssessment) but every LLM function bypasses them, retu...
16. **Unify 7 LLM steps into one declarative LlmStep** (maintenance)
   - Every LLM function in analyze.py, design.py, evaluate.py and reflect.py repeats the same ritual: a private _generate(provider,prompt,deterministic,coe...

### Accepted Ideas (3)
1. **Reconcile emitted keys with the evaluation models** (maintenance)
   - evaluate.py emits hasFindings on the evaluation dict and hasTransfers on the transfer dict, but neither key is declared on the CaseEvaluation or Trans...
2. **Stop silently dropping unparseable candidates in source** (functionality)
   - source_candidates in source.py wraps build_match_candidate in except Exception: continue, so any candidate whose payload fails CandidateProfileV2 vali...
3. **Promote comment-enums to one typed source of truth** (code_quality)
   - The four probe kinds (ambiguity|legacy_trap|verification_trap|underspecified) are hardcoded across ~7 sites � models.py docstrings, design.py coerce (...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513757_n62ur2k",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
