...
## Target Context
- Context ID: ctx_1780321513799_9ajd7xb
- Context Name: GitHub Code Analysis
...
**Context Description**:
Analyze a candidate's GitHub repositories — snapshot a repo and produce an AI code/skills assessment surfaced in a dedicated analysis panel.
...
### Pending Ideas (5)
1. **Give the analysis API a discriminated success/error envelope** (functionality)
   - The route returns the full analysis on success but also returns 200 with { error } when GitHub fails, when input is invalid, and even when githubAnaly...
2. **Reconcile the two divergent GitHub fetch layers** (maintenance)
   - repo-snapshot.ts opens with a comment claiming it Mirrors the fetch layer used by /api/github-analysis and uses GITHUB_TOKEN (same env var as the anal...
3. **Add a cached repo snapshot layer for GitHub analysis** (functionality)
   - Every analysis re-fetches the user, repos, per-repo languages, and the deep-review bundles with revalidate:0, and the Gemini review re-fetches commits...
4. **Extract a pure assessment core from the route monolith** (maintenance)
   - The 165-line POST in route.ts interleaves network I/O, ranking, language merging, complexity/contribution/job-fit signal building, Gemini orchestratio...
5. **Unify the two parallel GitHub REST clients** (maintenance)
   - app/api/github-analysis/route.ts and app/_lib/repo-snapshot.ts each reimplement the same GitHub client: fetch with headers, token resolution, README b...

### Accepted Ideas (5)
1. **Make the bursty commit-cadence heuristic explicit** (maintenance)
   - In fetchRepoSignals, bursty is computed as spanHours <= Math.max(6, times.length), which compares a duration in hours against a count of commits — a u...
2. **Define active vs recently-updated as named, documented rules** (maintenance)
   - metrics.activeRepos counts repos via isWithinMonths(pushed_at ?? updated_at, 12) while metrics.recentlyUpdatedRepos uses isWithinMonths(updated_at, 3)...
3. **Make Code-Aware Review honest about what it reads** (user_benefit)
   - The Code-Aware Review prompt in route.ts tells Gemini to decide which skills are demonstrably present in the code, and the UI labels the result Confir...
4. **Single-source the codeReview shape and validate Gemini output** (code_quality)
   - The codeReview payload is defined three times - the CodeReviewPayload TS type in route.ts, the zod block in schemas.ts, and the literal in the e2e fix...
5. **Fix false-positive substring skill matching in job fit** (code_quality)
   - buildJobFitSignals lowercases all repo metadata into one haystack and tests skill aliases with haystack.includes(alias). Short aliases produce phantom...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513799_9ajd7xb",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
