...
## Target Context
- Context ID: ctx_1780321513831_7b9o1lf
- Context Name: Workspace Shell & Shared UI
...
**Context Description**:
The studio shell that hosts every feature tab — workspace layout, sidebar navigation, tab routing, live-refresh, root layout/metadata and the shared UI primitives (badges, modal, markdown, icons) reused across features.
...
Found 11 active idea(s) (11 total, excluding rejected/implemented for brevity):

### Pending Ideas (4)
1. **Specify Markdown grammar and add safe link support** (user_benefit)
   - The Markdown renderer supports a subset, but the unsupported syntax is implicit: bracketed links, tables, blockquotes, nested lists, and images all re...
2. **Give useJsonFetch an explicit loading and abort contract** (maintenance)
   - useJsonFetch returns only data and error, so the common state where both are null conflates still-loading with a successful but empty or null body. Co...
3. **Lazy-load tab content via dynamic imports** (maintenance)
   - Workspace.tsx statically imports all 13 feature tabs plus the 6 simulation overlay components at module top, so the first paint of the default Pipelin...
4. **Collapse tab routing into one declarative registry** (maintenance)
   - Workspace.tsx hard-codes a 13-branch {navActive === "x" ? <X/> : null} ladder plus 13 static imports, while tabs.ts separately declares the same ids/l...

### Accepted Ideas (7)
1. **Add scope topics to the live-refresh event bus** (functionality)
   - notifyDataChanged broadcasts one global kp:data-changed event with no payload, so every mounted useLiveRefresh subscriber reloads on any mutation rega...
2. **Make the tab-to-component map exhaustive with fallback** (maintenance)
   - Workspace.tsx renders tab content through a long chain of navActive === id ternaries that must be hand-synced with the WorkspaceTabId union and NAV_GR...
3. **Codify the history-to-analyze tab alias contract** (maintenance)
   - The history id is a valid WorkspaceTabId accepted by isWorkspaceTabId, yet it has no entry in NAV_GROUPS or WORKSPACE_TABS and is silently remapped to...
4. **Lock the pure shared primitives behind golden tests** (code_quality)
   - The shared layer hides untested business logic in pure functions: Badge.ts token mappers (scoreToFitTier, fitTierToken) duplicate matching.pys 70/55 t...
5. **Ship the dark theme the token seam already enables** (functionality)
   - globals.css already carries a complete [data-theme="dark"] override seam � every surface resolves color through var(--color-*), and the comment states...
6. **Make live-refresh scoped and typed, not all-or-nothing** (maintenance)
   - live-refresh.ts fires one bare kp:data-changed event, so a mutation in the simulation driver forces every open subscriber � Analytics, Matrix, Pipelin...
7. **Unify the two duplicated workspace sidebars** (maintenance)
   - Workspace.tsx (client <button> tab-switch) and WorkspaceNav.tsx (server <Link> deep-link) duplicate the entire <aside>, the KP logo block, the NAV_GRO...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513831_7b9o1lf",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
