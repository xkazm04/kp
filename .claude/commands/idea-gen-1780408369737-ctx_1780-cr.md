...
## Target Context
- Context ID: ctx_1780321513583_btewh7b
- Context Name: Candidate Profile Builder
...
**Context Description**:
Build and edit a structured candidate profile from evidence (CV, links, manual fields). The resulting profile feeds matching, analysis and the pipeline.
...
### Pending Ideas (15)
1. **Resolve auto-detect default and dead routing signals** (functionality)
   - detect_archetype accepts yearsRelevantExperience and educationIsDominant, but ProfileTab never sends educationIsDominant and only collects yearsExperi...
2. **Define save semantics: edit/upsert vs always-insert** (functionality)
   - The context promises build AND edit a profile, but saveProfile() in db.ts is insert-only (new unique slug every Save), the ProfileTab form never loads...
3. **Make completeness score archetype-relative and explicit** (user_benefit)
   - completeness() in profile.py uses a different checklist and total weight per archetype (~9.0 for student/career_switcher vs ~7.5 for BAU), so a 70% st...
4. **Single source of truth for profile taxonomy enums** (maintenance)
   - EVIDENCE_KINDS, SKILL_LEVELS and the provenance vocabulary are declared three times: ProfileTypes.ts (frontend dropdowns), profile.py (Evidence/_KIND_...
5. **Reveal provenance-weighted skill strength in the result** (functionality)
   - The whole reason CandidateProfileV2 exists is that provenance discounts a claim (a thesis skill is not five years in production), and the model carrie...
6. **Edit a saved profile by rehydrating the intake form** (functionality)
   - The context mission is build AND edit, but saved profiles render as read-only chips with no way back into the form, and the GET /api/profile?id= endpo...
7. **Model the intake form as one CandidateProfileV2 draft** (maintenance)
   - ProfileTab.tsx spreads the profile across ~17 separate useState hooks, then manually reassembles them into a profile object inside build(). Collapse t...
8. **Promote the completeness checklist to a declarative rubric** (maintenance)
   - profile.py/_checklist() hardcodes which items matter per archetype with magic weights (1.0, 1.5, 2.0) and returns bare label strings the UI can only p...
9. **Generate the profile vocabulary from one taxonomy** (code_quality)
   - The candidate vocabulary (archetypes, evidence kinds, skill levels, provenance, role families, edu levels, seniorities) is declared at least three tim...
10. **Live match-readiness preview as you type** (ui)
   - Right now archetype, confidence and completeness only appear after clicking 'Check (preview)' in ProfileTab.tsx. Because profile_cli.py is pure logic ...
11. **Potential benchmarking against the talent pool** (user_benefit)
   - The app already seeds an enterprise-scale synthetic population (seed_candidates.py) and computes completeness plus a potential_score, but neither is c...
12. **Portable candidate-owned career passport** (functionality)
   - Profiles today are recruiter-side snapshots in the profiles table with no candidate ownership. Introduce a candidate-owned, living profile with a shar...
13. **Conversational evidence excavator for hidden strengths** (user_benefit)
   - completeness() in profile.py already returns the biggest missing gaps, but ProfileResultPanel only shows them as a static checklist. Replace that with...
14. **Verifiable evidence trust layer for provenance** (functionality)
   - PROVENANCE_WEIGHTS in taxonomy.py already discount a school-project skill versus production, but every claim is self-declared. Add a verification pass...
15. **One-paste evidence autofill from CV, GitHub & links** (functionality)
   - Today ProfileTab.tsx forces recruiters to hand-type every skill and evidence row. Add an 'Import evidence' path that accepts a CV upload, GitHub handl...

### Accepted Ideas (2)
1. **Add a real e2e smoke test for the profile builder** (maintenance)
   - e2e/profile-smoke.spec.ts is named for this context but actually drives the CV Analyze flow (attach CV, job description, Score Breakdown, GitHub) and ...
2. **Add real e2e coverage for the Profile Builder** (maintenance)
   - Despite its name, e2e/profile-smoke.spec.ts exercises the Analyze flow (CV upload, job description, GitHub) and never touches ProfileTab, /api/profile...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513583_btewh7b",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
