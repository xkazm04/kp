...
## Target Context
- Context ID: ctx_1780321513766_31r1bdx
- Context Name: Automation Orchestration
...
**Context Description**:
Run the end-to-end automated recruiting pipeline as scheduled or triggered passes — execute per-task automation, schedule runs and surface automation status across the funnel.
...
### Pending Ideas (9)
1. **Make the on-demand task route idempotent for sends** (functionality)
   - The /api/automation/[task] route is documented as the non-hardened twin of /api/tasks (tracked, dedup, refresh-safe). The gemini_cache only short-circ...
2. **Serialize the policy pass across its three triggers** (maintenance)
   - runAutomationPass can be launched by the Run pass button (/api/automation/run, no lock), a forced manual tick (/api/automation/schedule with tick:true...
3. **Enforce or delete the dead rematch and confidence knobs** (functionality)
   - AUTOMATION_SPEC.md promises a max-2 rematch cap and a confidence_low>=65 BAU advance gate, but automation.py never reads POLICY[rematch_max] or POLICY...
4. **Type the TS to Python automation entry contract** (code_quality)
   - listActiveEntriesForAutomation (TS) emits a camelCase snapshot that evaluate_entry (Python) reads field-by-field with entry.get(...), with the shape d...
5. **Express the policy stage machine as a transition table** (maintenance)
   - evaluate_entry in automation.py is a pure finite state machine hiding inside an if stage=='Sourced'/'AI-matched'/... chain. Lift it into an explicit S...
6. **Make POLICY thresholds a live, inspectable source of truth** (functionality)
   - The POLICY dict in automation.py is declared 'the only place rules live' yet is hardcoded and invisible at runtime � no one can see or tune what auto-...
7. **Unified automation activity stream across the funnel** (functionality)
   - Today only the policy pass is recorded in scheduler_runs; the 7 on-demand LLM tasks dissolve into per-entry pipeline_events with no rollup, and the ri...
8. **Split decide from apply for a true policy dry-run** (functionality)
   - runAutomationPass currently computes decisions in Python then immediately applies them in the same loop � advancing entries, REJECTING candidates, and...
9. **Collapse 7 automation tasks into one task descriptor** (maintenance)
   - In automation-run.ts each LLM task's identity is smeared across four sites: the AUTOMATION_VERSION map, the positional cache-key array (lines 56-58), ...

### Accepted Ideas (2)
1. **Give Python CLI errors honest HTTP status codes** (maintenance)
   - automation_cli.py hardcodes status 500 for every exception (the error envelope is error plus status 500), so a client error like job not found or an i...
2. **Hold, not auto-reject, an unscored AI-matched entry** (maintenance)
   - In evaluate_entry a Sourced entry with matchScore 0 or null holds (awaiting match score), but a BAU AI-matched entry with the same missing score is au...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513766_31r1bdx",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
