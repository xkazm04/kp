...
## Target Context
- Context ID: ctx_1780321513709_yr1hfry
- Context Name: Scheduling & Offers
...
**Context Description**:
Send interview-schedule invites and offer links via public token pages, let candidates pick slots, finalize offers and dispatch reminders.
...
### Pending Ideas (9)
1. **Stop the offers store from writing pipeline_entries directly** (code_quality)
   - offers-store.ts owns the offers table but reaches across to UPDATE pipeline_entries via markEntryStatus, while offer-finalize.ts also mutates pipeline...
2. **Give public links a real lifecycle: expiry and revocation** (functionality)
   - Offer and schedule tokens live forever and their status is a loose TEXT field (extended/accepted/declined, pending/confirmed) enforced only by convent...
3. **Unify the two slot models into one timezone-aware Slot** (maintenance)
   - There are two disconnected notions of an interview slot. The recruiter calendar (ScheduleTypes.ts) speaks abstract weekly labels ("Tue 14:00", TIMES=[...
4. **Mint public link tokens with a CSPRNG, not Math.random()** (code_quality)
   - Both stores mint candidate-facing tokens with Math.random().toString(36) (st-/tk- prefixes). These tokens are the ONLY gate on pages exposing salary, ...
5. **Collapse schedule-store and offers-store into one PublicLink core** (maintenance)
   - schedule-store.ts and offers-store.ts are the same pattern wearing two costumes: an isolated better-sqlite3 connection with identical WAL/busy_timeout...
6. **Add expiry/TTL to offer and schedule tokens** (functionality)
   - Neither offers (offers-store.ts) nor schedule invites (schedule-store.ts) have an expiry: both store created_at but every getOfferByToken/getScheduleI...
7. **Pin and surface the interview timezone on proposed slots** (user_benefit)
   - proposeSlots() in schedule-store.ts builds slots with slot.setHours(h, m) in the SERVER local timezone, then stores slot_at via toISOString() and labe...
8. **Define offer-accept partial-failure consistency contract** (maintenance)
   - In offer-finalize.ts respondToOffer(), markOfferResponded(token, accepted) runs FIRST, then actOnPipelineEntry(accept) and await dispatchOnboarding(hi...
9. **Unify the three divergent interview slot vocabularies** (maintenance)
   - Three incompatible slot formats coexist: the recruiter grid in ScheduleTypes.ts ("Tue 14:00", TIMES 09:00/10:30/11:00/14:00/15:30), proposeSlots() in ...

### Accepted Ideas (1)
1. **Validate confirmed slot against currently-proposed slots** (functionality)
   - The POST in app/api/schedule/[token]/route.ts trusts the client-supplied body.slot (label) and body.slotAt (ISO) verbatim - SchedulePicker sends both,...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513709_yr1hfry",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
