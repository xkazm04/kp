...
## Target Context
- Context ID: ctx_1780321513659_zsgw6ec
- Context Name: Voice Interview Runtime
...
**Context Description**:
Run real-time AI voice interviews with candidates (ElevenLabs / OpenAI realtime) — create, connect and complete sessions, replay and compare transcripts; includes the interview lab and public interview link page.
...
### Pending Ideas (14)
1. **Single-source the interviewer persona and company** (maintenance)
   - The You are male / Czech masculine-forms instruction is hardcoded in three places (interview-run.ts composeBrief, voice/index.ts defaultInterviewerIns...
2. **Make interview create and connect fail-closed** (maintenance)
   - In /api/interview/create, when no provider is requested and neither OpenAI nor ElevenLabs is configured, the provider resolution falls through to open...
3. **Define the interview session state machine** (maintenance)
   - The session status (created/in_progress/completed/failed) is a free-form TEXT column with literals scattered across db.ts, the API routes, and a separ...
4. **Surface when a candidate interview loses its CV grounding** (functionality)
   - buildGroundedInterview swallows prep failures (try/catch falls back to a generic brief with an empty runOfShow), and for ElevenLabs the per-candidate ...
5. **Make the Language toggle actually steer the interview** (functionality)
   - The candidate-facing Language selector (Auto/Ce�tina/English) is threaded from VoiceInterview.tsx through /api/interview/connect into both adapters, b...
6. **Decouple scorecard synthesis from the complete request** (code_quality)
   - /api/interview/complete runs runInterviewScorecard - an LLM automation task - synchronously inside the POST handler the browser awaits during hang-up,...
7. **Unify the two interview-question models** (functionality)
   - Two disconnected systems generate interview questions: the Python build_interview_kit (CV-gap questions with STAR scaffolds for candidate prep) and th...
8. **Make the provider the source of truth for transcripts** (code_quality)
   - Today the transcript is assembled in the browser and POSTed best-effort at hang-up, so a tab close, crash, or network drop loses the entire interview ...
9. **Complete the VoiceAdapter lifecycle abstraction** (maintenance)
   - The VoiceAdapter interface stops at connect(), so all post-connect logic (transcript streaming, teardown, finalize triggers) leaks into the 500-line V...
10. **Branded two-way candidate interview portal** (ui)
   - Evolve the tokenized /interview/[token] page from a bare screen into a branded, white-labeled candidate experience: company theming, a warm intro, and...
11. **Live interviewer copilot for human interviews** (functionality)
   - Extend the live transcript + grounded run-of-show + rubric into a copilot that rides alongside a HUMAN interviewer on Zoom/Teams calls. It surfaces th...
12. **Self-serve candidate interview rehearsal mode** (functionality)
   - Let candidates run a private practice voice interview against their own CV and a target job description before any real screen, reusing the voice runt...
13. **Hiring-outcome flywheel that re-calibrates scoring** (functionality)
   - Close the loop between interview transcripts/scorecards and downstream hire outcomes (offer accepted, 90-day retention, ramp performance). Feed those ...
14. **EU AI Act-ready interview fairness & audit layer** (user_benefit)
   - Build a compliance and fairness layer on top of the existing consent + transcript-only + fixed-rubric foundation (VoiceInterview.tsx, interview/compar...

### Accepted Ideas (2)
1. **Document the transcript truncation policy for scoring** (maintenance)
   - runInterviewScorecard slices the joined notes to 6000 chars and /api/interview/complete caps each turn text at 4000 chars, so a long interview is sile...
2. **Single source of truth for the interviewer persona** (maintenance)
   - The same Czech-first interviewer persona (warm, male masculine forms, AI disclosure, transcribed, under five minutes) is hand-built as joined string a...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513659_zsgw6ec",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
