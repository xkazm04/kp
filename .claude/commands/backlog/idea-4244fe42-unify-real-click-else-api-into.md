...
# Unify 'real-click-else-API' into one act() primitive
...
- **Generated**: 6/1/2026, 4:11:45 PM

## Description
The pattern 'try a real DOM click via clickEl(), and if it returns false log "(… not visible — via API)" and call the route directly' is copy-pasted across the source, interview, offer, and hired phases — ~10 near-identical lines each. Extract a single actViaClickOrApi(selector, {title, caption}, apiFallback) helper that owns the spotlight, the wait, the click, the fallback log, and the API call. Every phase then expresses its intent in one line.

## Reasoning
This is generative compression: one abstraction generates all the click-or-fallback cases, eliminating four sites where the logging text and fallback wiring can silently drift apart. It shrinks the provider and makes the genuine click-vs-fallback behavior obvious and testable.
