...
## Target Context
- Context ID: ctx_1780321513563_ujrno9h
- Context Name: Scoring & Extraction Engine (Python)
...
**Context Description**:
The Python job-fit pipeline: parses CV text, builds candidate profiles, scores fit vs a JD, detects archetypes, flags ATS issues, maps transferable skills and estimates salary bands using Gemini.
...
### Pending Ideas (12)
1. **Flag low-confidence archetype routing in results** (functionality)
   - In pipeline._v2_profile_from_payload the archetype is always auto-detected (self_declared=None) from best-effort Gemini booleans that are frequently n...
2. **Reconcile the two divergent seniority ladders** (maintenance)
   - Seniority is inferred in two places with different rules. pipeline._build_deterministic_evidence treats junior as a floor checked before medior (lead+...
3. **Make the CZK/month salary assumption explicit** (maintenance)
   - _salary_from_payload accepts whatever currency and period Gemini returns (defaulting to CZK/month), yet downstream logic silently assumes CZK monthly ...
4. **Deterministic no-LLM fallback analysis path** (functionality)
   - Today analyze_cv hard-depends on Gemini and raises if GEMINI_API_KEY is absent or the API stalls. The deterministic layer already computes nearly ever...
5. **Normalize CV text once via a Document value object** (maintenance)
   - Almost every function in taxonomy.py opens with text_n = _normalize(text); compact = _compact(text_n), recomputing an NFC+casefold pass over up to 2MB...
6. **Pydantic models as single source for Gemini schema** (maintenance)
   - The analysis contract is encoded three times: the prose ANALYSIS_RESPONSE_SCHEMA dict in gemini.py, the Pydantic models in models.py, and the hand-wri...
7. **Open the bilingual skill taxonomy as a platform API** (functionality)
   - The taxonomy graph in taxonomy.py (ancestors/descendants hierarchy, role-family votes, provenance weights, Czech+English surface forms) plus skill_mat...
8. **Glass-box interactive score explainer** (ui)
   - Surface the engines existing explainability as a drill-down UI: every score sub-total (experience/skills/role_seniority/education/traits) is clickable...
9. **EU AI Act fairness and explainability report** (user_benefit)
   - Candidate-scoring AI is high-risk under the EU AI Act. The engine already emits evidence_trace, sanity_checks, archetype reasons, per-skill provenance...
10. **Outcome-calibrated talent graph with memory** (functionality)
   - Each analyze_cv run is stateless today. Persist analyses keyed by candidate and role, then fold back real outcomes - was the candidate hired, did the ...
11. **Flip the engine into a candidate-facing CV coach** (user_benefit)
   - The pipeline already computes cv_rewrite_suggestions, must_prove_evidence, keyword coverage, archetype potential, transferable skills, salary expectat...
12. **Living Czech tech compensation benchmark engine** (functionality)
   - Every analysis already produces a grounded, source-cited CZK salary band plus deterministic anchor bands from data/salary_benchmarks.json. Capture eac...

### Accepted Ideas (5)
1. **Name and document the silent extraction caps** (maintenance)
   - Several magic caps silently drop data with no rationale or flag: detected_skills truncates to 30 in the pipeline (40 default in taxonomy), evaluate_ke...
2. **Source the present-year anchor, drop hardcoded 2026** (maintenance)
   - pipeline/jobfit/profiling.py hardcodes 2026 as the end year for ongoing roles (present/now/dosud) and as the clamp ceiling in _estimate_years. From 20...
3. **First-class fail-soft degradation collector** (code_quality)
   - pipeline.py repeats a degrade-and-flag pattern at least four times � try: build evidence_trace/interview_kit/keyword_coverage/v2_profile; except: set ...
4. **Unify salary-band rules into one invariant module** (code_quality)
   - _round_salary is duplicated verbatim in pipeline.py and insights.py, and the salary-band invariant (two positive numbers, min<=max, swap if reversed) ...
5. **Collapse duplicate taxonomy category scanners** (maintenance)
   - taxonomy.py carries four near-identical functions � has_seniority_lead_signal/senior/medior/junior � that differ only by a literal level string, plus ...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513563_ujrno9h",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
