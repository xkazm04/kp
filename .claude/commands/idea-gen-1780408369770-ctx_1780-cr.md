...
## Target Context
- Context ID: ctx_1780321513512_op07yqa
- Context Name: CV Analysis Workspace
...
**Context Description**:
Upload or paste a CV and a job description, then run an AI job-fit analysis with live scan progress. The primary intake surface (Analyze tab) for evaluating a single candidate against a role.
...
### Pending Ideas (14)
1. **Make JD source precedence explicit and validate jdSlug** (functionality)
   - When a JD file and JD text (typed or from a saved JD) are both present, cliArgs in analyze-run.ts silently prefers the file and the GitHub deep-dive e...
2. **Define and restore form state on refresh-resume** (user_benefit)
   - useAnalyzeForm persists only the analyze task id (kp.analyzeTaskId) and on reload calls resumeAnalysis, but the form inputs (cvFiles, JD, company, git...
3. **Bound client task polling with timeout and 404 handling** (functionality)
   - watchAnalysis polls /api/tasks/[id] in a while(true) loop every 1.5s with no max attempts, deadline, or 404 handling: a failed fetch hits if(!r.ok) co...
4. **Close empty-MIME gap between client and server gates** (maintenance)
   - upload-constraints.ts includes an empty string in ACCEPT_MIME because some browsers report an empty type for .md, and its header comment claims the cl...
5. **Drive analysis progress from real pipeline state** (maintenance)
   - watchAnalysis in AnalyzeApi.ts advances the six-stage strip (extract/gemini/profile/scoring/salary/insights) on a fixed 1800ms setInterval that is ful...
6. **Make the whole run refresh-safe, including GitHub** (functionality)
   - Refresh-survival is half-built: ANALYZE_TASK_KEY stashes only the main analyze task id, so after a reload resumeAnalysis restores the result but the c...
7. **Model JD/Company as one discriminated-union source** (code_quality)
   - The job description lives across three loosely-coupled fields (jobDescriptionFile, jobDescriptionText, selectedJdSlug) whose mutual exclusion is hand-...
8. **Unify the two document dropzones into one component** (maintenance)
   - AnalyzeFileDropZone and AnalyzeProfileInput are the same concept wearing two disguises: a validated document dropzone at multiplicity 1 (JD/company) v...
9. **Make pipeline progress real, not a 1800ms fake timeline** (functionality)
   - watchAnalysis (AnalyzeApi.ts) animates the six stages on a blind setInterval while the Python pipeline emits one final JSON blob. Meanwhile analyze-ru...
10. **Explainable streaming results: show your work** (ui)
   - Replace the simulated stage timeline in watchAnalysis (which fakes progress on a fixed timer) with genuine streamed partial results as the pipeline em...
11. **Shared calibrated Role Library with network effects** (functionality)
   - Promote the private saved-JD library into a shared, versioned library of role templates that carry a learned keyword/skill taxonomy plus the distribut...
12. **Compensation Intelligence built from every analysis** (user_benefit)
   - Aggregate the salary estimates every run already emits (role family x seniority x skills x company type -> band, with confidence and market evidence) ...
13. **Application Coach: iterative re-score improvement loop** (user_benefit)
   - Turn the one-shot job-fit report into a closed loop. Surface cvRewriteSuggestions, mustProveEvidence, and missing ATS keywords as inline, apply-and-re...
14. **Recruiter Pipeline: rank many candidates per role** (functionality)
   - Repoint the existing buildComparison engine (which already ranks up to 3 CV variants best-of-N) at a batch of distinct candidates evaluated against on...

### Accepted Ideas (2)
1. **Unify file validation across all CV intake paths** (maintenance)
   - Three intake paths treat files inconsistently: the empty AnalyzeFileDropZone runs validateUpload (extension+size), but the Replace path calls onFileCh...
2. **Tighten the AnalyzeApi seam: typed payload + stage SoT** (maintenance)
   - submitAnalysis takes six positional arguments (cvFiles, jdFile, jdText, companyFile, companyText, jdSlug) that are trivial to transpose, even though A...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513512_op07yqa",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
