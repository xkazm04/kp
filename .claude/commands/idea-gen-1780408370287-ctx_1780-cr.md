...
## Target Context
- Context ID: ctx_1780321513820_crdczuo
- Context Name: Data Layer, Schemas & Python Bridge
...
**Context Description**:
Core persistence and cross-language plumbing — the SQLite database and all table schemas, the Gemini response cache, the TS↔Python runner bridge, Zod schemas generated from Pydantic models, the async task runner, health probe, logging and shared utilities.
...
### Pending Ideas (10)
1. **Replace swallowed ALTER TABLEs with a migration ledger** (maintenance)
   - Schema evolution in ensureDb() is a pile of try/catch ALTER TABLE ADD COLUMN loops plus a guarded unique index -- unordered, untracked, and silent abo...
2. **Generalize gemini_cache into one LLM result cache** (functionality)
   - The content-addressed, versioned, TTL'd gemini_cache (db.ts + cache.ts) is bespoke to the analyze path, while the reasoning path keeps its OWN version...
3. **One typed envelope for the TS/Python CLI bridge** (functionality)
   - The bridge contract is implicit: python-runner.ts builds argv (buildCliArgs), scans stdout from the end for the last JSON object (parsePythonJson), an...
4. **Validate every persisted payload with Zod on read** (code_quality)
   - Only AnalysisResult is schema-checked today (codegen'd Python to Zod in schemas.generated.ts via codegen.py); every other payload_json -- JobRecord, p...
5. **Dissolve db.ts monolith into declarative table repos** (maintenance)
   - app/_lib/db.ts is a 2,449-line module where every table repeats one shape: inline DDL, indexed columns, a payload_json blob, a hand-written rowToX map...
6. **Document what degraded means in /api/health** (functionality)
   - The readiness probe returns ok or 503 but the definition of healthy is implicit: today it flags only seed errors and an empty jobs table, while ignori...
7. **Unify ID and token generation and collision contract** (maintenance)
   - Slug-backed tables use insertWithUniqueSlug, which retries on a UNIQUE collision, but tasks, interviews, lifecycles, postings, submissions, and outbox...
8. **Codify pipeline stage and approval state machine** (maintenance)
   - pipeline_entries stage, status, and approval_kind are free-form TEXT with no enum or CHECK, and the transition rules live implicitly inside actOnPipel...
9. **Define and validate the TS to Python bridge protocol** (maintenance)
   - The contract between the Node bridge (python-runner.ts) and the Python CLIs lives only in scattered comments: success is the last stdout line that par...
10. **Guard PROMPT_VERSION against silent schema drift** (maintenance)
   - PROMPT_VERSION in app/_lib/cache.ts is the sole cache key for analyze results, and its comment says to bump it whenever the Pydantic models, prompt, p...
...

{
  ...
  "project_id": "5928226b-548c-40e3-97fe-a9cecc59712a",
  "context_id": "ctx_1780321513820_crdczuo",
  "scan_type": "code_refactor",
  "category": "<category>",
  "title": "<title>",
