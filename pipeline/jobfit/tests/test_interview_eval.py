...
Pins three things:
  ...
  3. the interviewer briefs come ONLY from the committed snapshot
     (eval/interview_briefs.json, generated by `npm run interview:briefs` and held to the
     production TS builders by app/_lib/voice/interview-brief-snapshot.test.ts): no Python
     prompt literals, no node subprocess, no silent fallback to another brief kind.
"""
...
class TestBriefSnapshot(unittest.TestCase):
    """The briefs come from the committed snapshot (eval/interview_briefs.json), never from a
    Python re-render or a node subprocess. The snapshot's faithfulness to the production TS
    builders is proven on the node side (app/_lib/voice/interview-brief-snapshot.test.ts);
    this side proves Python reads it, substitutes the role, and refuses what it cannot serve."""

    GROUNDED = "grounded_senior_strong"

    @classmethod
    def setUpClass(cls):
        import json

        cls.snapshot = json.loads(ie.BRIEF_SNAPSHOT_PATH.read_text(encoding="utf-8"))

    def _template(self, kind):
        return self.snapshot["kinds"][kind]["template"]

    def test_grounded_renders_from_the_snapshot_with_no_node_process(self):
        import subprocess
        from unittest import mock

        scn = _scn(name=self.GROUNDED, brief="grounded", role_line="Senior Backend Engineer", seniority="senior")
        boom = AssertionError("a brief render spawned a process")
        with mock.patch.object(subprocess, "run", side_effect=boom), \
                mock.patch.object(subprocess, "Popen", side_effect=boom):
            brief = ie.render_brief(scn)
            for kind in ie.BRIEF_KINDS:
                ie.render_brief(_scn(brief=kind, role_line="a role"))
                ie.render_brief(_scn(brief=kind, role_line=""))
        expected = self._template("grounded").replace(self.snapshot["sentinel"], "Senior Backend Engineer")
        self.assertEqual(brief, expected)
        self.assertNotIn(self.snapshot["sentinel"], brief)
        self.assertIn("Recent backend ownership", brief)      # the fixture's run of show
        self.assertIn("Design trade-offs", brief)
        self.assertIn("Do not give feedback, scores", brief)  # the compliance clause

    def test_empty_role_uses_the_recorded_fallback(self):
        for kind in ie.BRIEF_KINDS:
            with self.subTest(kind=kind):
                fallback = self.snapshot["kinds"][kind]["fallbackRole"]
                self.assertTrue(fallback)
                self.assertEqual(
                    ie.render_brief(_scn(brief=kind, role_line="")),
                    self._template(kind).replace(self.snapshot["sentinel"], fallback),
                )

    def test_default_and_student_wrappers_read_the_snapshot(self):
        sentinel = self.snapshot["sentinel"]
        self.assertEqual(default_brief("a senior role"), self._template("default").replace(sentinel, "a senior role"))
        self.assertEqual(student_brief("a junior role"), self._template("student").replace(sentinel, "a junior role"))

    def test_unknown_brief_kind_raises_naming_it(self):
        with self.assertRaises(ValueError) as ctx:
            ie.render_brief(_scn(name="p", brief="panel"))
        self.assertIn("panel", str(ctx.exception))
        self.assertIn(ie.REGENERATE_COMMAND, str(ctx.exception))

    def test_an_unknown_kind_is_an_errored_row_not_a_default_brief(self):
        # The row keeps its label, and it is NOT scored as if the default brief had run.
        rows = ie.run_scenarios([_scn(name="p", brief="panel")], provider=object(), max_workers=1)
        self.assertEqual(len(rows), 1)
        self.assertTrue(rows[0].errored)
        self.assertEqual(rows[0].brief, "panel")
        self.assertEqual(rows[0].turns, [])
    ...
    def test_loader_refuses_a_different_schema(self):
        import copy

        bad = copy.deepcopy(self.snapshot)
        bad["schema"] = self.snapshot["schema"] + 1
        with self.assertRaises(ValueError) as ctx:
            ie.load_brief_snapshot(self._write(bad))
        self.assertIn("schema", str(ctx.exception))
        self.assertIn(ie.REGENERATE_COMMAND, str(ctx.exception))

    def test_loader_refuses_a_missing_kind_naming_it(self):
        import copy
    ...
    def test_loader_refuses_a_template_without_the_sentinel(self):
        import copy

        bad = copy.deepcopy(self.snapshot)
        bad["kinds"]["case"]["template"] = "no role here"
        with self.assertRaises(ValueError) as ctx:
            ie.load_brief_snapshot(self._write(bad))
        self.assertIn("case", str(ctx.exception))

    def test_the_committed_snapshot_loads(self):
        briefs = ie.load_brief_snapshot()
        self.assertEqual(sorted(briefs), sorted(ie.BRIEF_KINDS))

    def test_no_python_port_and_no_node_bridge_remain(self):
        src = Path(ie.__file__).read_text(encoding="utf-8")
        self.assertNotIn("PERSONA_", src)
        self.assertNotIn("scripts/interview-brief.ts", src)
        self.assertNotIn("scripts/interview-brief-grounded.ts", src)
        self.assertNotIn("import subprocess", src)

    def test_grounded_scenarios_load(self):
        names = {s.name for s in load_scenarios()}
        self.assertIn(self.GROUNDED, names)
        briefs = sorted({s.brief for s in load_scenarios()})
        self.assertEqual([b for b in briefs if b not in ie.BRIEF_KINDS], [], briefs)

    def test_briefs_carry_the_compliance_clauses(self):
        for kind in ie.BRIEF_KINDS:
            with self.subTest(kind=kind):
                brief = ie.render_brief(_scn(brief=kind, role_line="a role"))
                self.assertIn("Do not give feedback, scores", brief)
                self.assertIn("Do not assume the candidate's language", brief)
...
class TestOptimizer(unittest.TestCase):
    ...
    def test_optimizer_rejects_a_train_only_improvement(self):
        ...
        def fake_run_scenarios(scenarios, provider, brief_transform=None):
