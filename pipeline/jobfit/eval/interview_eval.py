...
# ---------------------------------------------------------------------------
# Interviewer briefs — read from a COMMITTED SNAPSHOT of the production TS builders.
#
# eval/interview_briefs.json is generated by scripts/interview-briefs-snapshot.ts
# (`npm run interview:briefs`), which renders every brief kind through the real builders
# (defaultInterviewerInstructions, studentInterviewerInstructions,
# caseGroundedInterviewerInstructions, and composeBrief via buildGroundedInterview over a
# seeded fixture entry) with a sentinel where the role goes. The node:test
# app/_lib/voice/interview-brief-snapshot.test.ts holds the file to those builders, so
# every eval mode — offline, CI, operator sim, optimizer — scores the text the live agent
# receives. There is no Python re-render of the prompt and no node process spawned here: a
# brief-copy change is written once, in TS, and fails that test until it is regenerated.
# ---------------------------------------------------------------------------

BRIEF_SNAPSHOT_PATH = Path(__file__).with_name("interview_briefs.json")
BRIEF_SNAPSHOT_SCHEMA = 1
BRIEF_KINDS = ("default", "student", "case", "grounded")
REGENERATE_COMMAND = "npm run interview:briefs"
...
class BriefTemplate:
    ...
    def render(self, role_line: str) -> str:
        # The builders' own `role || <fallback>`, recorded per kind by the generator.
        return self.template.replace(self.sentinel, role_line or self.fallback_role)


def load_brief_snapshot(path: Path = BRIEF_SNAPSHOT_PATH) -> dict[str, BriefTemplate]:
    """Read and validate the brief snapshot. Refuses — naming what is wrong and the
    regenerate command — a schema it does not know, a missing kind, or a template the role
    cannot be substituted into. Validated at import, like thresholds.py's records."""
    data = json.loads(Path(path).read_text(encoding="utf-8"))
    where = f"{Path(path).name} (regenerate with `{REGENERATE_COMMAND}`)"
    if not isinstance(data, dict) or data.get("schema") != BRIEF_SNAPSHOT_SCHEMA:
        got = data.get("schema") if isinstance(data, dict) else None
        raise ValueError(f"{where}: schema {got!r}, this eval reads schema {BRIEF_SNAPSHOT_SCHEMA}")
    sentinel = data.get("sentinel")
    if not isinstance(sentinel, str) or not sentinel:
        raise ValueError(f"{where}: no `sentinel`")
    kinds = data.get("kinds") if isinstance(data.get("kinds"), dict) else {}
    missing = [k for k in BRIEF_KINDS if k not in kinds]
    if missing:
        raise ValueError(f"{where}: missing brief kind(s) {', '.join(missing)}")
    out: dict[str, BriefTemplate] = {}
    for kind in BRIEF_KINDS:
        entry = kinds[kind] if isinstance(kinds[kind], dict) else {}
        template, fallback = entry.get("template"), entry.get("fallbackRole")
        if not isinstance(template, str) or sentinel not in template:
            raise ValueError(f"{where}: kind {kind!r} has no template carrying the sentinel {sentinel}")
        if not isinstance(fallback, str) or not fallback:
            raise ValueError(f"{where}: kind {kind!r} has no `fallbackRole`")
        out[kind] = BriefTemplate(template=template, fallback_role=fallback, sentinel=sentinel)
    return out


_BRIEFS = load_brief_snapshot()


def render_brief(scenario: "Scenario") -> str:
    """The production brief for the scenario's kind, with its role line substituted. A kind
    the snapshot does not carry RAISES, naming it — never a silent fallback to another brief
    (a row labelled `grounded` must not be scored against the default agent)."""
    template = _BRIEFS.get(scenario.brief)
    if template is None:
        raise ValueError(
            f"interview_eval: brief kind {scenario.brief!r} (scenario {scenario.name}) is not in "
            f"{BRIEF_SNAPSHOT_PATH.name}, which carries {', '.join(BRIEF_KINDS)}; add it to "
            f"scripts/interview-briefs-snapshot.ts and run `{REGENERATE_COMMAND}`"
        )
    return template.render(scenario.role_line)


def default_brief(role_line: str) -> str:
    """The quick-screen brief (defaultInterviewerInstructions) for ``role_line``."""
    return _BRIEFS["default"].render(role_line)


def student_brief(role_line: str) -> str:
    """The early-career brief (studentInterviewerInstructions) for ``role_line``."""
    return _BRIEFS["student"].render(role_line)
...
def simulate(
    scenario: Scenario, provider: ClaudeCliProvider, brief_transform=None
...
def run_scenarios(
    ...
    brief_transform=None,
) -> list[Row]:
    def _one(s: Scenario) -> Row:
        try:
            turns, ended, errored = simulate(s, provider, brief_transform)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--briefs", choices=["port", "ts"], default=None,
                        help="DEPRECATED no-op, kept one release: every run reads the committed production-brief "
                             "snapshot (eval/interview_briefs.json; regenerate with `npm run interview:briefs`).")
    ...
    if args.briefs is not None:
        sys.stderr.write(
            f"interview_eval: --briefs {args.briefs} is a no-op — every run reads the production "
            f"brief snapshot ({BRIEF_SNAPSHOT_PATH.name}; regenerate with `{REGENERATE_COMMAND}`)\n"
        )
    ...
    if args.no_llm:
        ...
        if not provider.available():
            ...
            rows = run_scenarios(scenarios, provider)
